...
if (!function_exists('importSqlParser')) {
  function importSqlParser()
  {
    spl_autoload_register(function ($className) {
      if (strpos(strtolower($className), 'phpsql') !== false && !class_exists($className)) {
        ...
        $fileName = CFS_HELPERS_DIR . $fileName;
    ...
    if (!class_exists($class, true)) {
      if (!$path) {
        ...
        $path = CFS_HELPERS_DIR . $classFile . '.php';
      }
...
if (!function_exists('toeGetClassNameCfs')) {
  function toeGetClassNameCfs($class)
  {
    ...
    } elseif (class_exists(CFS_CLASS_PREFIX . $class)) {
...
if (!function_exists('prepareParamsCfs')) {
  function prepareParamsCfs(&$d = [], &$options = [])
  {
    if (!empty($d['params'])) {
