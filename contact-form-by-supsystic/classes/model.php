...
abstract class modelCfs extends baseObjectCfs
{
  ...
  public function getFromTbl($params = [])
  {
  ...
  protected function _clearQuery($params = [])
  {
    $clear = isset($params['clear']) ? $params['clear'] : [];
    if (!is_array($clear)) {
      $clear = [$clear];
    }
  ...
  public function supGetById($id, $customTable = '')
  {
    ...
    return $res = $wpdb->get_results(
      $wpdb->prepare("SELECT {$this->_idField} FROM %1s WHERE id = %s", $tableName, $id),
