...
class langCfs
{
  ...
  public static function extract($d = ['dir' => '', 'langCfs' => ''])
  {
    $data = [];
    if (isset($d['dir']) && !empty($d['dir'])) {
      $langDirPath = $d['dir'];
    } elseif (isset($d['langCfs']) && !empty($d['langCfs'])) {
      $langDirPath = CFS_LANG_DIR . $d['langCfs'] . DS;
    } else {
      $langDirPath = CFS_LANG_DIR . CFS_WPLANG . DS;
    }

    if (is_dir($langDirPath)) {
      $dh = opendir($langDirPath);
      while (($file = readdir($dh)) !== false) {
        if (!in_array($file, ['.', '..']) && !empty($file)) {
