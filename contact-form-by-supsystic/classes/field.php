...
class fieldCfs
{
   ...
   */
  public $adapt = ['htmlCfs' => '', 'dbFrom' => '', 'dbTo' => ''];
  /**
   ...
   */
  public function __construct($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $adaption = [], $validate = '', $description = '')
  {
    $this->name = $name;
    $this->html = $html;
    $this->type = $type;
    ...
    if (method_exists('htmlCfs', $this->html)) {
      ...
      if ($this->name == 'default_value') {
        ...
        if (!empty($optionsFromDb)) {
          $options = [0 => __('Select', CFS_LANG_CODE)];
  ...
  public function viewField($name, $fieldValue = '')
  {
    ...
    $params = ['optionsCfs' => $options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked];
    $output = '';
    if (method_exists('htmlCfs', $method)) {
      $output .= htmlCfs::$method($name, $params);
      $output .= htmlCfs::hidden('extra_field[' . $this->name . ']', ['value' => $this->id]);
    }
    return $output;
  }
  ...
  public function processParams($tag, $id)
  {
    ...
    if ($this->name == 'params') {
      ...
      if ($tag > 0 || $id == 0) {
        ...
        $output .= htmlCfs::text('params[width]', ['value' => $width]) . '<br />';
        $output .= __('height', CFS_LANG_CODE) . ':<br />';
        $output .= htmlCfs::text('params[height]', ['value' => $height]) . '<br />';
        $output .= '</div>';
      }
  ...
  public function prepareConfigOptions($xml)
  {
    ...
    foreach ($config->params->param as $param) {
      ...
      $config_params[$name] = ['type' => $type, 'label' => $label, 'helperCfs' => $helper, 'moduleCfs' => $module, 'values' => $values, 'default' => $default, 'description' => $description];
    }
    ...
    if (!empty($configOptions)) {
      ...
      foreach ($params as $param) {
        ...
        foreach ($configOptions as $key => $value) {
          ...
          $htmlParams = ['value' => $fieldValue, 'optionsCfs' => $options];
          if ($method == 'checkbox') {
            $htmlParams['value'] = 1;
