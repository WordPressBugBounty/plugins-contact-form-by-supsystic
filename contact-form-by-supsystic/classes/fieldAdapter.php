...
class fieldAdapterCfs
{
  ...
  public static function taxDataHtml($field)
  {
    ...
    if (!empty($categories)) {
      ...
      foreach ($categories as $c) {
        $cOptions[] = ['id' => $c->term_taxonomy_id, 'text' => $c->cat_name, 'checked' => in_array($c->term_taxonomy_id, $field->value['categories'])];
      }
    }
    if (!empty($brands)) {
      ...
      foreach ($brands as $b) {
        $bOptions[] = ['id' => $b->term_taxonomy_id, 'text' => $b->cat_name, 'checked' => in_array($b->term_taxonomy_id, $field->value['brands'])];
      }
    }
    return '<div>' .
      __('Apply To', CFS_LANG_CODE) .
      '
            <div id="tax_address">
                <b>' .
      __('Address', CFS_LANG_CODE) .
      '</b><br />
                ' .
      __('Destination', CFS_LANG_CODE) .
      ':' .
      htmlCfs::checkboxlist('params[dest]', ['optionsCfs' => $listOfDest]) .
      '<br />
                ' .
      __('Country', CFS_LANG_CODE) .
      ':' .
      htmlCfs::countryList('params[country]', ['notSelected' => true, 'value' => $field->value['country']]) .
      '<br />
            </div>
            <div id="tax_category">
                <b>' .
      __('Categories', CFS_LANG_CODE) .
      '</b><br />
                ' .
      (empty($cOptions) ? __('You have no categories', CFS_LANG_CODE) : htmlCfs::checkboxlist('params[categories][]', ['optionsCfs' => $cOptions])) .
      '<br />
                    <b>' .
      __('Brands', CFS_LANG_CODE) .
      '</b><br />
                ' .
      (empty($bOptions) ? __('You have no brands', CFS_LANG_CODE) : htmlCfs::checkboxlist('params[brands][]', ['optionsCfs' => $bOptions])) .
      '<br />
            </div>
            <div>' .
      __('Tax Rate', CFS_LANG_CODE) .
      ': ' .
      htmlCfs::text('params[rate]', ['value' => $field->value['rate']]) .
      '</div>
            <div>' .
      __('Absolute', CFS_LANG_CODE) .
      ': ' .
      htmlCfs::checkbox('params[absolute]', ['checked' => $field->value['absolute']]) .
      '</div>
        </div>';
  }
  ...
  public static function getCountries($notSelected = false, $listBy = 'id')
  {
    $options = [];
    if (empty(self::$countries)) {
      self::$countries = self::getCachedCountries();
  ...
  public static function getStates($notSelected = false)
  {
    ...
    if (empty($options[$notSelected])) {
      ...
      if ($notSelected) {
        ...
        $options[$notSelected][0] = ['name' => __($notSelectedLabel), 'country_id' => null];
      }
  ...
  public static function getFieldAttributes($params)
  {
    ...
    if (!empty($params->attr)) {
      foreach ($params->attr as $key => $value) {
        $output .= __($key) . ':<br />';
        $output .= htmlCfs::text('params[attr][' . $key . ']', ['value' => $value]) . '<br />';
      }
    } else {
      $output .= __('class', CFS_LANG_CODE) . ':<br />';
      $output .= htmlCfs::text('params[attr][class]', ['value' => '']) . '<br />';
      $output .= __('id', CFS_LANG_CODE) . ':<br />';
      $output .= htmlCfs::text('params[attr][id]', ['value' => '']) . '<br />';
    }
    return $output;
  }
  /**
  ...
  public static function getCachedCountries($clearCache = false)
  {
    ...
    $data = $wpdb->get_results("SELECT id, name, iso_code_2, iso_code_3 FROM {$wpdb->prefix}cfs_countries", ARRAY_A);
  ...
  public static function getCachedStates($clearCache = false)
  {
    if (empty(self::$states) || $clearCache) {
      self::$states = frameCfs::_()
        ->getTable('states')
        ->leftJoin(frameCfs::_()->getTable('countries'), 'country_id')->getAll('toe_states.id,
					toe_states.name,
					toe_states.code,
