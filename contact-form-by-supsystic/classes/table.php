...
abstract class tableCfs
{
  ...
  public static function getInstance($table = '')
  {
    ...
    if (!isset($instances[$table])) {
      $class = 'table' . strFirstUp($table) . strFirstUp(CFS_CODE);
      ...
      if ($id) {
        ...
        foreach ($data as $field) {
          ...
          foreach ($field as $k => $v) {
            if (isset($this->_fields[$k])) {
              $row[$k] = toeCreateObjCfs('fieldCfs', [$this->_fields[$k]->name, $this->_fields[$k]->html, $this->_fields[$k]->type, $this->_fields[$k]->default, $this->_fields[$k]->label, $this->_fields[$k]->maxlen, $this->_fields[$k]->description]);
  ...
  {
    if ($transform) {
      return dbCfs::prepareQuery($this->_table);
    } else {
      return $this->_table;
    }
  }
  ...
  public function store($data, $method = 'INSERT', $where = '')
  {
    ...
    if (empty($fields)) {
      $this->_addError(__('Nothing to update', CFS_LANG_CODE));
      return false;
    }
    ...
    if (dbCfs::query($query)) {
      ...
      $this->_addError(CFS_TEST_MODE ? dbCfs::getError() : __('Database error. Please contact your developer.', CFS_LANG_CODE));
    }
    return false;
  }
  ...
  /**
   * Add new fieldCfsCfs for children table (@see class field)
  ...
  {
    $this->_fields[$name] = toeCreateObjCfs('fieldCfs', [$name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description]);
    return $this;
  }
  /**
  ...
  public function exists($value, $field = '')
  {
    ...
    global $wpdb;
    $res = $wpdb->get_var($wpdb->prepare('SELECT * FROM wp_cfs_modules WHERE %1s = %s', $field, $value));
    return $res;
    //return dbCfs::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
  }
      ...
      switch ($f->type) {
        case 'date':
          if ($d[$key] == '0000-00-00' || empty($d[$key])) {
            ...
            $d[$key] = date(CFS_DATE_FORMAT, dbCfs::dateToTime($d[$key]));
          }
          break;
        case 'int':
        case 'tinyint':
  ...
  {
    return dbCfs::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
  }
