...
abstract class moduleCfs extends baseObjectCfs
{
  ...
  public function __construct($d /*, $params = array()*/)
  {
    ...
    if (isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
      ...
      $this->setExternalDir(utilsCfs::getExtModDir($d['ex_plug_dir']));
      $this->setExternalPath(utilsCfs::getExtModPath($d['ex_plug_dir']));
    }
  }
  ...
  public function getModDir()
  {
    if (empty($this->_externalDir)) {
      return CFS_MODULES_DIR . $this->getCode() . DS;
    } else {
  ...
  public function getModPath()
  {
    if (empty($this->_externalPath)) {
      return CFS_MODULES_PATH . $this->getCode() . '/';
    } else {
  ...
  protected function _createController()
  {
    ...
    if (file_exists($this->getModDir() . 'controller.php')) {
      $className = '';
      if (importCfs($this->getModDir() . 'controller.php')) {
        $className = toeGetClassNameCfs($this->getCode() . 'Controller');
      }
      if (!empty($className)) {
  ...
  protected function _createHelper()
  {
    ...
    if (file_exists($this->getModDir() . 'helper.php')) {
      ...
      importClassCfs($helper, $this->getModDir() . 'helper.php');
