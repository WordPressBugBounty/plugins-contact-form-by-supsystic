...
abstract class viewCfs extends baseObjectCfs
{
  ...
  public function addCfsNonce()
  {
    if (!$this->_nonceIsInited) {
      $this->_nonceIsInited = true;
      if (is_admin()) {
        $cfs_nonce = wp_create_nonce('cfs_nonce');
        $jsData = [
          'cfs_nonce' => $cfs_nonce,
        ];
        frameCfs::_()->addScript('nonceCfs', CFS_JS_PATH . 'nonce.js');
        frameCfs::_()->addJSVar('nonceCfs', 'CFS_NONCE', $jsData);
      } else {
        $cfs_nonce = wp_create_nonce('cfs_nonce_frontend');
        $jsData = [
          'cfs_nonce_frontend' => $cfs_nonce,
        ];
        frameCfs::_()->addScript('nonceCfs', CFS_JS_PATH . 'nonce.js');
        frameCfs::_()->addJSVar('nonceCfs', 'CFS_NONCE_FRONTEND', $jsData);
      }
    }
  }
   ...
   */
  public function displayWidgetForm($data = [], $widget = [], $formTpl = 'form')
  {
