      ...
      for (var i = 0; i < keysAssign.length; i++) {
        switch (i) {
          ...
          case 2:
            ...
            break;
          case 3:
            if (!toReturn[keysAssign[i - 3]][keysAssign[i - 2]][keysAssign[i - 1]][keysAssign[i]]) toReturn[keysAssign[i - 3]][keysAssign[i - 2]][keysAssign[i - 1]][keysAssign[i]] = [];
            break;
          case 4:
            if (!toReturn[keysAssign[i - 4]][keysAssign[i - 3]][keysAssign[i - 2]][keysAssign[i - 1]][keysAssign[i]]) toReturn[keysAssign[i - 4]][keysAssign[i - 3]][keysAssign[i - 2]][keysAssign[i - 1]][keysAssign[i]] = [];
            break;
        }
      }
...
jQuery.fn.serializeAnythingCfs = function (addData, returnArray) {
  ...
  jQuery.each(els, function () {
    if (this.name && !this.disabled && (this.checked || /select|textarea/i.test(this.nodeName) || /text|hidden|password|number/i.test(this.type))) {
      ...
      if (returnArray) {
        toReturn[this.name] = val;
      } else toReturn.push(encodeURIComponent(this.name) + '=' + encodeURIComponent(val));
    }
  });
...
function toeRedirect(url, newWnd) {
  if (newWnd) {
    ...
    if (win) {
      win.focus();
    } // Browser blocked new window showing
...
jQuery.fn.toeRebuildSelect = function (data, useIdAsValue, val) {
  if (jQuery(this).tagName() == 'SELECT' && typeof data == 'object') {
    if (jQuery(data).length > 0) {
      ...
      if (typeof useIdAsValue == 'undefined') useIdAsValue = false;
      var selected = '';
      for (var id in data) {
        selected = '';
        if (val && ((useIdAsValue && id == val) || data[id] == val)) selected = 'selected';
...
jQuery.fn.getClassId = function (pref, test) {
  ...
  if (strpos(classId, ' ')) classId = classId.substr(0, strpos(classId, ' '));
      ...
      if (count && count > list.length && this.perPage) {
        for (var i = 1; i <= Math.ceil(count / this.perPage); i++) {
          ...
          if (newPageId != this.page) {
            newElement.attr('href', '#' + newPageId).click(function () {
          ...
          if (i % 20 == 0 && i) this.paging.append('<br />');
        }
      }
    }
  };
  if (this.list) this.draw(this.list, this.count);
}
...
function isNumericCfs(n) {
  if (jQuery.isNumeric) return jQuery.isNumeric(n);
...
function toNumberCurrencyCfs(str) {
  if (!str) return false;
  var regex = /[+-]?\d+(\.\d+)?/,
    nums = str.match(regex).map(function (v) {
      return parseFloat(v);
    });
  if (!isNaN(nums[0])) {
    return {
...
function cfsGetTxtEditor(id) {
  if (typeof tinyMCE !== 'undefined' && tinyMCE && tinyMCE.get(id) && !jQuery('#' + id).is(':visible') && tinyMCE.get(id).getDoc && typeof tinyMCE.get(id).getDoc == 'function' && tinyMCE.get(id).getDoc()) return tinyMCE.get(id);
  return null;
}
