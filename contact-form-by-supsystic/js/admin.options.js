...
window.onbeforeunload = function () {
  ...
  if (cfsAdminFormChanged.length) return 'Some changes were not-saved. Are you sure you want to leave?';
};
...
function changeAdminFormCfs(formId) {
  if (jQuery.inArray(formId, cfsAdminFormChanged) == -1) cfsAdminFormChanged.push(formId);
}
...
function cfsInitStickyItem() {
  jQuery(window).scroll(function () {
    ...
    if (wasSticking) {
      if (jQuery('#cfsFormGoToTop').length) jQuery('#cfsFormGoToTop').show();
    } else if (wasUnSticking) {
      if (jQuery('#cfsFormGoToTop').length) jQuery('#cfsFormGoToTop').hide();
    }
  });
}
