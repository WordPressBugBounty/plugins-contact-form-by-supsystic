if (typeof CFS_DATA == 'undefined') var CFS_DATA = {};
if (isNumber(CFS_DATA.animationSpeed)) CFS_DATA.animationSpeed = parseInt(CFS_DATA.animationSpeed);
else if (jQuery.inArray(CFS_DATA.animationSpeed, ['fast', 'slow']) == -1) CFS_DATA.animationSpeed = 'fast';
...
jQuery.fn.sendFormCfs = function (params) {
  ...
  if (msgEl && !params.btn) {
    jQuery(msgEl).removeClass('cfsSuccessMsg').removeClass('cfsErrorMsg');
    if (!params.btn) {
  ...
  else if (typeof ajaxurl == 'undefined') url = CFS_DATA.ajaxurl;
  else url = ajaxurl;
...
function toeProcessAjaxResponseCfs(res, msgEl, form, sentFromForm, params) {
  ...
  if (typeof res == 'object') {
    if (res.error) {
      if (msgEl) {
        jQuery(msgEl).removeClass('cfsSuccessMsg').addClass('cfsErrorMsg');
      }
      var errorsArr = [];
      for (var name in res.errors) {
        ...
        if (jQuery('.cfsErrorForField.toe_' + nameToClassId(name) + '').exists())
          jQuery('.cfsErrorForField.toe_' + nameToClassId(name) + '')
            .show()
      ...
      if (errorsArr.length && params.btn && jQuery.fn.dialog && !msgEl) {
        jQuery('<div title="' + toeLangCfs('Really small warning :)') + '" />')
          .html(errorsArr.join('<br />'))
          .appendTo('body')
          .dialog({
            modal: true,
            width: '500px',
      ...
      if (msgEl) {
        jQuery(msgEl).removeClass('cfsErrorMsg').addClass('cfsSuccessMsg');
...
function toeOptionCfs(key) {
  if (CFS_DATA.options && CFS_DATA.options[key]) return CFS_DATA.options[key];
  return false;
}
function toeLangCfs(key) {
  if (CFS_DATA.siteLang && CFS_DATA.siteLang[key]) return CFS_DATA.siteLang[key];
  return key;
}
function toePagesCfs(key) {
  if (typeof CFS_DATA != 'undefined' && CFS_DATA[key]) return CFS_DATA[key];
  return false;
}
/**
...
function toeOptTimeoutHideDescriptionCfs() {
  ...
  setTimeout(function () {
    if (!jQuery('#cfsOptDescription').attr('toeFixTip')) toeOptHideDescriptionCfs();
  }, 500);
}
/**
...
function toeOptShowDescriptionCfs(description, x, y, moveToLeft) {
  if (typeof description != 'undefined' && description != '') {
    ...
    else jQuery('#cfsOptDescription').css('left', x + 10);
