...
class supsystic_promoControllerCfs extends controllerCfs
{
  ...
  public function sendContact()
  {
    ...
    if (!$res->error()) {
      ...
      foreach ($fields as $fName => $fData) {
        if (in_array($fName, ['name', 'email', 'subject'])) {
          continue;
        }
        if ($fName == 'category') {
  ...
  public function getPermissions()
  {
    return [
      CFS_USERLEVELS => [
        CFS_ADMIN => ['welcomePageSaveInfo', 'sendContact', 'addNoticeAction', 'addStep', 'closeTour', 'addTourFinish', 'sendSubscribeMail', 'sendSubscribeRemind', 'sendSubscribeDisable'],
      ],
    ];
  }
}
