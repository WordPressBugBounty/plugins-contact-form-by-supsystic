...
class templatesCfs extends moduleCfs
{
  ...
  {
    $this->_styles = [
      'styleCfs' => ['path' => CFS_CSS_PATH . 'style.css', 'for' => 'admin'],
      'supsystic-uiCfs' => ['path' => CFS_CSS_PATH . 'supsystic-ui.css', 'for' => 'admin'],
      'dashicons' => ['for' => 'admin'],
      'icheck' => ['path' => CFS_CSS_PATH . 'jquery.icheck.css', 'for' => 'admin'],
  ...
  public function loadGoogleFont($font)
  {
    ...
    if (!isset($loaded[$font])) {
      frameCfs::_()->addStyle('google.font.' . str_replace([' '], '-', $font), 'https://fonts.googleapis.com/css?family=' . urlencode($font));
      $loaded[$font] = 1;
    }
  }
