	...
	<div id="cfsFormSubmitEditTabMain" class="cfsTabContent nav-tab-active">
		<table class="form-table cfsFormSubmitOptsTbl" style="width: 100%">
			<tr>
				...
				<td>
					<?php echo htmlCfs::text('params[tpl][form_sent_msg]', ['value' => $this->form['params']['tpl']['form_sent_msg']]); ?>
				</td>
			</tr>
			<tr>
				<th scope="row">
				...
				<td>
					<?php echo htmlCfs::colorpicker('params[tpl][form_sent_msg_color]', [
       'value' => isset($this->form['params']['tpl']['form_sent_msg_color']) ? $this->form['params']['tpl']['form_sent_msg_color'] : '#4ae8ea',
			...
			<tr>
				...
				<td>
					<?php echo htmlCfs::checkboxHiddenVal('params[tpl][hide_on_submit]', [
       ...
       __('If you want - you can redirect user after Form was submitted. Just enter required Redirect URL here - and each time after Form will be submitted - user will be redirected to that URL. Just leave this field empty - if you don\'t need this functionality in your Form.', CFS_LANG_CODE),
     ); ?>"></i>
				</th>
				<td>
					<?php echo htmlCfs::text('params[tpl][redirect_on_submit]', [
       ...
       'attrs' => 'placeholder="http://example.com" style="width: 100%;"',
					...
					<label style="float: left; margin-right: 5px;">
						<?php echo htmlCfs::checkbox('params[tpl][redirect_on_submit_new_wnd]', [
        'checked' => htmlCfs::checkedOpt($this->form['params']['tpl'], 'redirect_on_submit_new_wnd'),
      ]); ?>
						<?php _e('Open in a new window (tab)', CFS_LANG_CODE); ?>
					</label>
					<label style="float: left;" class="supsystic-tooltip" title="<?php echo esc_html(
       sprintf(
         __(
           'After user will enter all data in contact form, and form will be submitted - he will be redirected to page with his submitted data. Enter in the text field above - link to page, where you inserted shortcode [%s] - and check results. Don\'t forget to enable "Save contacts data" option too.',
           CFS_LANG_CODE,
         ),
         CFS_SHORTCODE_SUBMITTED,
       ),
     ); ?>">
						<?php echo htmlCfs::checkbox('params[tpl][redirect_to_submitted]', [
        'checked' => htmlCfs::checkedOpt($this->form['params']['tpl'], 'redirect_to_submitted'),
			...
			<tr>
				<th scope="row">
					...
					<i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(
       __('By default email data from form will be listed in one column. But you can enable this option - and all data will be listed as they are in form: if in row there are 2 fields - 2 fields will be in email, etc.', CFS_LANG_CODE),
     ); ?>"></i>
				</th>
				<td>
					<?php echo htmlCfs::checkbox('params[tpl][email_form_data_as_tbl]', [
       ...
       'value' => isset($this->form['params']['tpl']['test_email']) ? $this->form['params']['tpl']['test_email'] : $this->adminEmail,
			...
			<tr>
				...
				<td>
					<?php echo htmlCfs::checkbox('params[tpl][save_contacts]', [
       'checked' => htmlCfs::checkedOpt($this->form['params']['tpl'], 'save_contacts'),
					...
					<span class="cfsContactExportCfsBtnShell">
						...
						<label class="supsystic-tooltip" title="<?php _e('Delimiter for CSV file columns', CFS_LANG_CODE); ?>">
							...
							<?php echo htmlCfs::text('params[tpl][exp_delim]', [
         ...
         'attrs' => 'style="max-width: 40px;" id="cfsContactsExportDelimTxt"',
			...
			<tr>
				<th scope="row">
					...
					<i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(
       __('By default - we will send field labels + values like:<br /><b>Field Label</b>: Field Value<br />But if you need to receive only values - you can disable sending Field Values here - just check this checkbox and it will done.', CFS_LANG_CODE),
     ); ?>"></i>
				</th>
				<td>
					<?php echo htmlCfs::checkbox('params[tpl][dsbl_send_labels]', [
       'checked' => htmlCfs::checkedOpt($this->form['params']['tpl'], 'dsbl_send_labels'),
			...
			<tr>
				<th scope="row">
					...
					<i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(
       sprintf(__('Enable Facebook Pixel and Custom conversion. More about this feature and it\'s integration - <a href="%s" target="_blank">here</a>.', CFS_LANG_CODE), 'https://www.facebook.com/business/help/952192354843755?helpref=faq_content'),
     ); ?>"></i>
					<?php if (!$this->isPro) { ?>
				...
				<td>
					<?php echo htmlCfs::checkbox('params[tpl][enb_fb_convert]', [
       ...
       'attrs' => 'data-switch-block="fbConvert" class="cfsProOpt"',
			...
			<tr data-block-to-switch="fbConvert">
				...
				<td>
					<?php echo htmlCfs::textarea('params[tpl][fb_convert_base]', [
       'value' => isset($this->form['params']['tpl']['fb_convert_base']) ? $this->form['params']['tpl']['fb_convert_base'] : '',
        ...
        CFS_LANG_CODE,
			...
			<table class="form-table cfsFormSubmitToTbl" style="width: 100%">
				<tr>
					...
					<td>
						<?php echo htmlCfs::text('params[submit][][to]', [
        'value' => $this->adminEmail,
				...
				<tr>
					...
					<td>
						<?php echo htmlCfs::text('params[submit][][from]', [
        'value' => $this->adminEmail,
      ]); ?>
					</td>
				</tr>
