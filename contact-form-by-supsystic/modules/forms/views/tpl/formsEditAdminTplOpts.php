...
<?php echo htmlCfs::hidden('params[enableForMembership]', [
...
<table class="form-table" style="width: auto;">
	<tr>
		...
		<td class="col-w-1perc">
			<?php echo htmlCfs::text('params[tpl][width]', ['value' => $this->form['params']['tpl']['width']]); ?>
		</td>
		<td class="col-w-1perc" colspan="3">
			<label style="margin-right: 10px;" class="supsystic-tooltip" title="<?php _e('Max width for percentage - is 100', CFS_LANG_CODE); ?>">
				<?php echo htmlCfs::radiobutton('params[tpl][width_measure]', ['value' => '%', 'checked' => htmlCfs::checkedOpt($this->form['params']['tpl'], 'width_measure', '%')]); ?>
				<?php _e('Percents', CFS_LANG_CODE); ?>
			</label>
			<label>
				<?php echo htmlCfs::radiobutton('params[tpl][width_measure]', ['value' => 'px', 'checked' => htmlCfs::checkedOpt($this->form['params']['tpl'], 'width_measure', 'px')]); ?>
    ...
    ?>
			...
			<td class="col-w-1perc">
				<?php echo htmlCfs::selectbox('params[tpl][bg_type_' . $i . ']', ['options' => $this->bgTypes, 'value' => $this->form['params']['tpl']['bg_type_' . $i], 'attrs' => 'data-iter="' . $i . '" class="cfsBgTypeSelect"']); ?>
			</td>
			<td class="col-w-1perc cfsBgTypeShell cfsBgTypeShell_<?php echo $i; ?> cfsBgTypeImgShell_<?php echo $i; ?>">
				<?php echo htmlCfs::imgGalleryBtn('params[tpl][bg_img_' . $i . ']', ['onChange' => 'cfsShowImgPrev', 'attrs' => 'data-iter="' . $i . '" class="button button-sup-small"', 'value' => $this->form['params']['tpl']['bg_img_' . $i]]); ?>
			</td>
			...
			<td class="col-w-1perc cfsBgTypeShell cfsBgTypeShell_<?php echo $i; ?> cfsBgTypeColorShell_<?php echo $i; ?>" style="line-height: 40px;">
				<?php echo htmlCfs::colorpicker('params[tpl][bg_color_' . $i . ']', ['value' => $this->form['params']['tpl']['bg_color_' . $i]]); ?>
			</td>
		</tr>
	<?php } ?>
	<?php
/*TODO: add this to PRO maybe?>
		<tr>
			<th scope="row" class="col-w-1perc">
