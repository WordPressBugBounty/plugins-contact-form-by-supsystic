...
class formsViewCfs extends viewCfs
{
  ...
  public function getTabContent()
  {
    ...
    frameCfs::_()->addJSVar('admin.forms.list', 'cfsTblDataUrl', uriCfs::mod('forms', 'getListForTbl', ['reqType' => 'ajax']));
  ...
  public function getAddNewTabContent()
  {
    ...
    if ($changeFor) {
      $originalForm = $this->getModel()->supGetById($changeFor);
      ...
      dispatcherCfs::addFilter('mainBreadcrumbs', [$this, 'modifyBreadcrumbsForChangeTpl']);
    }

    $this->assign('list', dispatcherCfs::applyFilters('showTplsList', $this->getModel()->getFormsTplList()));
  ...
  {
    $form = $this->getModel()->supGetById($id);
    if (empty($form)) {
      return __('Cannot find required Form', CFS_LANG_CODE);
    }
    dispatcherCfs::doAction('beforeFormEdit', $form);

    dispatcherCfs::addAction('afterAdminBreadcrumbs', [$this, 'showEditFormFormControls']);
    dispatcherCfs::addAction('adminBreadcrumbsClassAdd', [$this, 'adminBreadcrumbsClassAdd']);
    if (empty($form['ab_id'])) {
      dispatcherCfs::addFilter('mainBreadcrumbs', [$this, 'changeMainBreadCrumbsClb']);
    }
    ...
    frameCfs::_()->addJSVar('admin.forms.edit', 'frontendStyles', $frontendStyles);
    ...
    $this->assign('csvExportUrl', uriCfs::mod('forms', 'exportCsv', ['id' => $id]));
    ...
    $this->assign('previewUrl', uriCfs::mod('forms', 'getPreviewHtml', ['id' => $id]));
    ...
    uasort($tabs, [$this, 'sortEditFormTabsClb']);
  ...
  {
    return [$crumbs[count($crumbs) - 1]]; // Get rid of all other breadcrumbs - leave space on this page for other important things (buttons, etc.)
  }
  public function showEditFormFormControls()
  {
    ...
    $this->assign('pluginInstallUrl', add_query_arg(['s' => 'Membership by Supsystic', 'tab' => 'search', 'type' => 'term'], admin_url('plugin-install.php')));
  ...
  public function getMainFormSubmitOptsTab()
  {
    ...
    foreach ($postTypes as $key => $value) {
      if (!in_array($key, ['attachment', 'revision', 'nav_menu_item'])) {
    ...
    $allCategories = [];
    global $wpdb;
    foreach ($postTypesForCategoriesList as $categoryId) {
      $allCategories[] = $wpdb->get_row($wpdb->prepare("SELECT t.term_id, t.name FROM {$wpdb->terms} t INNER JOIN {$wpdb->term_taxonomy} tt ON t.term_id = tt.term_id WHERE tt.taxonomy = %s ORDER BY t.name ASC", $categoryId), ARRAY_A);
    }
    ...
    if (!empty($allCategories)) {
      foreach ($allCategories as $c) {
        if (!empty($c['term_id']) && !empty($c['name'])) {
    ...
    $this->assign(
      ...
      frameCfs::_()
        ->getModule('options')
        ->getTabUrl($this->getCode() . '_contacts', ['id' => $this->form['id']]),
    );
    $this->assign('regRolesForSelect', $this->getAvailableUserRolesForSelect());
    $this->assign('submitOptsAddTabs', [
      'cfsFormSubmitEditTabPublish' => [
      ...
      'cfsFormSubmitEditTabRegistration' => [
  ...
  public function getMainFormFieldsTab()
  {
    ...
    if ($isGoogleMapsAvailable) {
      $allGoogleMaps = frameGmp::_()
        ->getModule('gmap')
        ->getModel()
        ->getAllMaps(['simple' => true]);
      $allGoogleMapsForSelect = [];
  ...
  public function getMainFormStatisticsOptsTab()
  {
    ...
    $stats = $statModel->getAllForFormSorted($this->form['id'], ['group' => $group]);
    if (!empty($stats)) {
  ...
  {
    return [
      'supTablesUi' => CFS_CSS_PATH . 'suptablesui.min.css',
  ...
  public function showForm($params)
  {
    ...
    if (!empty($form)) {
      ...
      return $this->generateHtml($form, ['frontend' => true]) . '<span style="display: none !important;" class="cfsFormDesc">' . base64_encode(json_encode($this->_prepareForFront($form))) . '</span>';
    }
  ...
  private function _prepareForFront($form)
  {
    ...
    $removeParamsKeys = [
      'sub_aweber_listname',
      'sub_aweber_adtracking',
      'sub_mailchimp_api_key',
      'sub_mailchimp_lists',
      'sub_ar_form_action',
      'sub_sga_id',
      'sub_sga_list_id',
      'sub_sga_activate_code',
      'sub_gr_api_key',
      'sub_ac_api_url',
      'sub_ac_api_key',
      'sub_ac_lists',
  ...
  public function generateHtml($form, $params = [])
  {
    ...
    if (!empty($form['html']) && !is_array($form['html'])) {
      $form['html'] = str_replace('\r\n', '', $form['html']);
    }
    if (!empty($form['css']) && !is_array($form['css'])) {
      $form['css'] = str_replace('\r\n', '', $form['css']);
      $form['css'] = str_replace("\'", "'", $form['css']);
      $form['css'] = str_replace('\"', '"', $form['css']);
    }
    ...
    $form['html'] = str_replace('col-md-', 'supMd', $form['html']);
    $form['html'] = str_replace('col-xs-', 'supXs', $form['html']);
    $form['html'] = str_replace('col-sm-', 'supSm', $form['html']);
    $form['html'] = str_replace('col-lg-', 'supLg', $form['html']);
    $form['html'] = str_replace('row', 'supRow', $form['html']);
  ...
  private function _generateCommonFormCss($form)
  {
    ...
    if (isset($form['params'], $form['params']['tpl'], $form['params']['tpl']['form_sent_msg_color']) && !empty($form['params']['tpl']['form_sent_msg_color'])) {
  ...
  {
    $res = '';
    $res .= htmlCfs::hidden('mod', ['value' => 'forms']);
    $res .= htmlCfs::hidden('action', ['value' => 'contact']);
    $res .= htmlCfs::hidden('id', ['value' => $form['id']]);
    $res .= htmlCfs::hidden('_wpnonce', ['value' => wp_create_nonce('contact-' . $form['id'])]);
  ...
  public function generateFields($form)
  {
    ...
    if (isset($form['params']['fields']) && !empty($form['params']['fields'])) {
      ...
      foreach ($form['params']['fields'] as $f) {
        ...
        if (empty($name) && !$isHtmlDelim && !$isGoogleMap) {
          continue;
        }
        $id = '';
        ...
        if ($isHtmlDelim) {
          ...
          if (class_exists('frameGmp')) {
            $mapId = (int) $f['value'];
            if ($mapId) {
              $inputHtml = frameGmp::_()
                ->getModule('gmap')
                ->drawMapFromShortcode(['id' => $mapId]);
            }
          } elseif (frameCfs::_()->getModule('user')->isAdmin()) {
            // Show errors - only if user is admin, for usual visitors there will be just no field
            $inputHtml = sprintf(
              __('To use this field type you need to have installed and activated <a href="%s" target="_blank">Google Maps Easy</a> plugin - it\'s Free! Just install it <a class="button" target="_blank" href="%s">here.</a>', CFS_LANG_CODE),
              'https://wordpress.org/plugins/google-maps-easy/',
              admin_url('plugin-install.php?tab=search&s=Google+Maps+Easy'),
            );
          }
        } else {
          $htmlParams = ['attrs' => 'data-name="' . $name . '"'];
          $label = $f['label'];
          ...
          $showAsList = $fieldListSupported || in_array($htmlType, ['countryList', 'countryListMultiple', 'wooattrs', 'wpcategories']);
          $showAsOneCheck = in_array($htmlType, ['checkbox', 'radiobutton', 'checkboxsubscribe']);
          $isButton = in_array($htmlType, ['submit', 'reset', 'button']);
          $isRadioCheckList = in_array($htmlType, ['radiobuttons', 'checkboxlist']);
          ...
          if (isset($f['value']) && (!empty($f['value']) || strlen($f['value']) > 0)) {
            if (isset($f['value_preset']) && $f['value_preset'] && $addFieldsMod && method_exists($addFieldsMod, 'generateValuePreset')) {
          ...
          if (strpos($fieldWrapper, '[field_id]') !== false && !$isRadioCheckList) {
            // Need field ID
          ...
          if (!$isHidden) {
            ...
            // Wrap it
            $classes = [$fieldCommonClasses, 'cfsField_' . $htmlType];
            $replaceFrom = ['[field]', '[field_shell_classes]', '[field_shell_styles]', '[field_html]', '[field_id]'];
            $replaceTo = [$inputHtml, 'class="' . implode(' ', $classes) . '"', '', $htmlType, $id];
            if (!$insertLabelInternal) {
              $replaceFrom[] = '[label]';
              $replaceTo[] = $isButton ? '' : $label;
            }

            $inputHtml = str_replace($replaceFrom, $replaceTo, $fieldWrapper);
            ...
            $inputHtml = str_replace($replaceFrom, $replaceTo, $inputHtml);
          }
        }
        $bsClassId = isset($f['bs_class_id']) && !empty($f['bs_class_id']) ? (int) $f['bs_class_id'] : 12;
        $added = false;
        if (!$isHidden) {
          $inputHtml = '<div class="supSm' . $bsClassId . ' cfsFieldCol" data-name="' . $name . '" data-type="' . $htmlType . '">' . $inputHtml . '</div>'; // Bootstrap col wrapper
          if ($bsClassId < 12) {
        ...
        if (!$added) {
          // New row
          $rows[] = ['id' => $bsClassId, 'cols' => [$inputHtml], 'is_hidden' => $isHidden];
        }
      }
      foreach ($rows as $r) {
        $cols = implode('', $r['cols']);
        if (isset($r['is_hidden']) && $r['is_hidden']) {
          // No wrappers for hidden fields
          $resHtml .= $cols;
        } else {
          $resHtml .= '<div class="supRow cfsFieldsRow">' . $cols . '</div>';
        }
      }
    }
    return $resHtml;
  ...
  private function _replaceTagsWithTwig($string, $form)
  {
    ...
    $replaceFrom = ['SHELL_ID', 'ID', 'endif', 'else'];
    $replaceTo = [$form['view_html_id'], $form['view_id'], '{% endif %}', '{% else %}'];
  ...
  protected function _initTwig()
  {
    if (!$this->_twig) {
      ...
      $this->_twig->addFunction(new Twig_SimpleFunction('adjust_brightness', [$this, 'adjustBrightness']));
      $this->_twig->addFunction(new Twig_SimpleFunction('adjust_opacity', [$this, 'adjustOpacity']));
      $this->_twig->addFunction(new Twig_SimpleFunction('hex_to_rgba_str', 'utilsCfs::hexToRgbaStr'));
    }
  }
  public function getContactsTabContent($fid = 0)
  {
    ...
    frameCfs::_()->addJSVar('admin.forms.contacts.list', 'cfsTblDataUrl', uriCfs::mod('forms', 'getContactsListForTbl', ['reqType' => 'ajax', 'fid' => $fid]));
    ...
    if (!empty($allForms)) {
      ...
      foreach ($allForms as $form) {
        ...
        if (isset($form['params']['fields']) && !empty($form['params']['fields'])) {
          if (count($formsFields) < 5) {
            // More fields will look like very unpreatty
            foreach ($form['params']['fields'] as $f) {
              $htmlType = $f['html'];
              if (in_array($htmlType, ['submit', 'reset', 'button', 'recaptcha', 'htmldelim', 'googlemap', 'textarea', 'selectlist', 'radiobutton', 'radiobuttons', 'checkbox', 'checkboxlist', 'checkboxsubscribe', 'countryList', 'countryListMultiple'])) {
                continue;
              }
              ...
              $formsFields[$sendName] = ['label' => $f['label'] ? $f['label'] : $f['placeholder'], 'html' => $html];
            }
          }
        }
      }
