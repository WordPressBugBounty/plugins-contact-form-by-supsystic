...
class formsControllerCfs extends controllerCfs
{
  ...
  protected function _prepareListForTbl($data)
  {
    if (!empty($data)) {
      if ($this->_forceModelName == 'contacts') {
        ...
        foreach ($data as $i => $v) {
          $data[$i]['label'] = '<a class="" href="' . $this->getModule()->getEditLink($data[$i]['id']) . '">' . esc_html($data[$i]['label']) . '&nbsp;<i class="fa fa-fw fa-pencil" style="margin-top: 2px;"></i></a>';
          $conversion = 0;
  ...
  {
    if ($this->_prevFormId) {
      $form = $this->getModel()->supGetById($this->_prevFormId);
  ...
  public function exportForDb()
  {
    ...
    $tblsCols = [
      '@__forms' => ['unique_id', 'label', 'active', 'original_id', 'params', 'html', 'css', 'sort_order', 'date_created', 'is_pro', 'img_preview'],
    ];
    if ($forPro) {
      echo 'db_install=>';
  ...
  private function _makeExportQueriesLogicForPro($table, $cols)
  {
    ...
    echo str_replace(['@__'], '', $table) . '|' . base64_encode(utilsCfs::serialize($res));
  }
  ...
  public function contact()
  {
    ...
    frameCfs::_()
      ...
      ->getModel()
      ->add(['id' => $id, 'type' => 'submit']);

    if ($this->getModel()->contact($data)) {
      $lastForm = $this->getModel()->getLastForm();
      $successMsg = isset($lastForm['params']['tpl']['form_sent_msg']) ? $lastForm['params']['tpl']['form_sent_msg'] : __('Thank you for contacting us!', CFS_LANG_CODE);
      $successMsg = dispatcherCfs::applyFilters('contactSuccessMsg', $successMsg, $lastForm);
      $res->addMessage($successMsg);
      $redirectUrl = isset($lastForm['params']['tpl']['redirect_on_submit']) && !empty($lastForm['params']['tpl']['redirect_on_submit']) ? $lastForm['params']['tpl']['redirect_on_submit'] : false;
  ...
  {
    return ['save', 'removeGroup', 'getListForTbl', 'saveAsCopy', 'remove', 'createFromTpl', 'changeTpl'];
  }
  public function getPermissions()
  {
    return [
      CFS_USERLEVELS => [
        CFS_ADMIN => ['createFromTpl', 'getListForTbl', 'remove', 'removeGroup', 'clear', 'save', 'getPreviewHtml', 'exportForDb', 'changeTpl', 'saveAsCopy', 'switchActive', 'outPreviewHtml', 'updateLabel', 'exportCsv', 'getContactsListForTbl', 'getContactDetails', 'removeContactsGroup'],
      ],
    ];
  }
}
