...
cfsForm.prototype._bindFieldsMatchValidation = function () {
  ...
  if ($bindToFields && $bindToFields.length) {
    ...
    $bindToFields
      .change(function () {
        // Validate on field change itself
        var $this = jQuery(this),
          eqToName = $this.data('equal-to');
        cfsCheckFieldsMatchValidation($this, self.getFieldInput(eqToName), self);
      })
      .each(function () {
        // Also repeat validation when binded-to field was changed: user enter incorrect value to field itself, but then set to binded field same value - and error should be gone
        var $this = jQuery(this),
          eqToName = $this.data('equal-to'),
...
cfsForm.prototype._checkUpdateRecaptcha = function () {
  ...
  if (reCaptchFields && reCaptchFields.length) {
    // if reCaptcha exists
...
cfsForm.prototype._checkTimePickers = function () {
  ...
  if (fields) {
    for (var i = 0; i < fields.length; i++) {
      if (fields[i].html == 'time' && fields[i].time_format && fields[i].time_format != 'am_pm') {
        var $input = this.getFieldInput(fields[i].name);
        if ($input && $input.length) {
          if ($input.hasClass()) {
            // Time input was inited
    ...
    for (var i = 0; i < this._data.params.fields.length; i++) {
      if (parseInt(this._data.params.fields[i].mandatory)) {
        ...
        if (
          ...
          var label = this._data.params.fields[i].label ? this._data.params.fields[i].label : this._data.params.fields[i].placeholder,
...
cfsForm.prototype.onBeforeSubmit = function (clb) {
  if (typeof clb === 'function') this._beforeSubmitClbs.push(clb);
};
...
cfsForm.prototype._bindSubmit = function () {
  var self = this;
  this._$.find('.csfForm:not(.cfsSubmitBinded)')
    .submit(function () {
        ...
        onSuccess: function (res) {
          ...
          if (!res.error) {
            ...
            if (hideOnSubmit === false || parseInt(hideOnSubmit)) {
              ...
              if (docScrollTop > formShellTop) {
                // If message will appear outside of user vision - let's scroll to it
...
cfsForm.prototype.extractFormData = function () {
  ...
  return jQuery('<div />')
    .append(
      jQuery('<div id="' + this._data.tpl.view_html_id + '" />')
        .append($chatBlock)
        .append($style)
    )
    .html();
