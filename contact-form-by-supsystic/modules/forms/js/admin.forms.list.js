jQuery(document).ready(function () {
  ...
  jQuery('#cb_' + tblId + '').change(function () {
    jQuery(this).attr('checked') ? jQuery('#cfsFormRemoveGroupBtn').removeAttr('disabled') : jQuery('#cfsFormRemoveGroupBtn').attr('disabled', 'disabled');
  });
  jQuery('#cfsFormRemoveGroupBtn').click(function () {
    ...
    var confirmMsg = listIds.length > 1 ? toeLangCfs('Are you sur want to remove ' + listIds.length + ' Forms?') : toeLangCfs('Are you sure want to remove "' + formsLabel + '" Form?');
    if (confirm(confirmMsg)) {
      jQuery.sendFormCfs({
        btn: this,
        data: { mod: 'forms', action: 'removeGroup', listIds: listIds, _wpnonce: CFS_NONCE['cfs_nonce'] },
