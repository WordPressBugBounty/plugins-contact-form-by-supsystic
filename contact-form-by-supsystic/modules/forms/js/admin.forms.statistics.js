...
function cfsRefreshCharts() {
  for (var elmId in g_cfsChart) {
    if (g_cfsChart[elmId] && g_cfsChart[elmId].chart && g_cfsChart[elmId].tbl && !g_cfsChart[elmId].viewportRefreshed) {
...
function cfsInitChartsControls() {
  ...
  jQuery('#cfsFormStatistics')
    .find('input[name=stat_from_txt],input[name=stat_to_txt]')
    .datepicker({
      onSelect: function () {
        var chartId = jQuery(this).data('chart');
        jQuery('#cfsFormStatistics')
          .find('.cfsStatClearDateBtn[data-chart="' + chartId + '"]')
          .show();
  ...
  // Chart type controls
  jQuery('#cfsFormStatistics')
    .find('.cfsStatChartTypeBtn')
    .click(function () {
      var chartId = jQuery(this).data('chart'),
        type = jQuery(this).data('type');
      jQuery('#cfsFormStatistics')
        .find('.cfsStatChartTypeBtn[data-chart="' + chartId + '"]')
        .removeClass('focus');
  ...
  jQuery('#cfsFormStatistics')
    .find('.cfsStatChartGroupBtn')
    .click(function () {
      var chartId = jQuery(this).data('chart'),
        group = jQuery(this).data('stat-group'),
        self = this;
      jQuery('#cfsFormStatistics')
        .find('.cfsStatChartGroupBtn[data-chart="' + chartId + '"]')
        .removeClass('focus');
      cfsGetStats(
        chartId,
        function (data) {
  ...
  jQuery('.cfsChartArea').each(function () {
    ...
    if (!sdChartType) sdChartType = 'line'; // By default
    if (sdChartType) {
      jQuery('#cfsFormStatistics')
    ...
    if (!sdChartGroup) sdChartGroup = 'day';
    if (sdChartGroup) {
      jQuery('#cfsFormStatistics')
