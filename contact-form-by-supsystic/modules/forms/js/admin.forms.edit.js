...
jQuery(document).ready(function () {
  jQuery('#cfsFormEditTabs').wpTabs({
    ...
    change: function (selector) {
      ...
      if (selector == '#cfsFormStatistics') {
        // Hide preview for statistics tab
  ...
  jQuery('#cfsFormEditForm').submit(function () {
    ...
    if (!cfsSaveWithoutPreviewUpdate) cfsShowPreviewUpdating();
  ...
  jQuery('#cfsFormShortcodeExampleSel')
    .change(function () {
      jQuery('.cfsFormWhereShowBlock')
        .hide()
        .filter('[data-for="' + jQuery(this).val() + '"]')
        .show();
    })
    .trigger('change');
  // Fallback for case if library was not loaded
  ...
  jQuery('.cfsFormRemoveBtn').click(function () {
    if (confirm(toeLangCfs('Are you sure want to remove this Form?'))) {
      jQuery.sendFormCfs({
        btn: this,
        data: { mod: 'forms', action: 'remove', id: cfsForm.id, _wpnonce: CFS_NONCE['cfs_nonce'] },
        onSuccess: function (res) {
          if (!res.error) {
  ...
  jQuery('#cfsFormEditForm')
    .find('[name="params[tpl][width]"]')
    .keyup(function () {
  ...
  jQuery('#cfsFormEditForm')
    .find('[name="params[tpl][width_measure]"]')
    .change(function () {
  ...
  jQuery('#supsystic-breadcrumbs').bind('startSticky', function () {
    ...
    jQuery('#cfsFormMainControllsShell')
      .css('padding-right', currentPadding + 200)
      .attr('data-padding-changed', 'padding is changed in admin.forms.edit.js');
  });
  ...
  // Edit Form Label
  jQuery('#cfsFormEditableLabelTxt')
    .blur(function () {
  ...
  jQuery('#cfsFormEditForm [name="params[tpl][save_contacts]"]')
    .change(function () {
  ...
  jQuery('input[type=checkbox][data-switch-block]')
    .change(function () {
...
function cfsAdjustFormsEditTabs(requring) {
  jQuery('#cfsFormEditTabs .supsystic-always-top').outerWidth(jQuery('#cfsFormEditTabs').width()).attr('data-code-tip', 'Width was set in admin.forms.edit.js - cfsAdjustFormsEditTabs()');
...
function cfsMakeAutoUpdate(delay) {
  ...
  if (cfsFormSaveTimeout) clearTimeout(cfsFormSaveTimeout);
