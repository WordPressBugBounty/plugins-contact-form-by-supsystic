var g_cfsFieldsFrame = {
  ...
  init: function () {
    ...
    if (!CFS_DATA.isPro) {
      this._$editWnd.find('[name="value_preset"]').change(function () {
        if (jQuery(this).val()) cfsFillAndShowMainPromoWnd(jQuery(this).parents('td:first'));
      });
    }
  ...
  _rebuildIconsSlider: function (filter) {
    ...
    for (var iconId in icons) {
      ...
      $slide.append('<div class="cfsFieldIconBtn" data-icon="' + iconId + '" ><i class="fa fa-' + iconId + ' fa-2x"></i></div>');
  ...
  _showIconsSlider: function (field) {
    ...
    if (!sliderCreated) {
      ...
      jQuery('#cfsFieldIconSearchInp').keyup(function (e) {
        ...
        this._cfsSearchTimeout = setTimeout(
    ...
    if (field && field.icon_class) {
      ...
      $fieldIcons.find('div[data-icon="' + field.icon_class + '"]').addClass('active');
    } else {
      ...
      if ($input && $input.length) {
        if (this._fields[key]) {
          switch (this._fields[key].html) {
            case 'checkbox':
              parseInt(field[key]) ? $input.prop('checked', 'checked') : $input.removeProp('checked');
              break;
            case 'colorpicker':
              if ($input.wpColorPicker) {
                $input.wpColorPicker('color', field[key]);
              } else $input.val(field[key]);
              break;
            default:
              $input.val(field[key]);
              break;
  ...
  _addListOpt: function (opt, params) {
    ...
    if (!params.$optExRow) {
      // This mea that we add only one opt, not adding batch of them
      this._updateOptsListSortOrder();
    }
  },
  ...
  _getFieldFields: function (htmlCode) {
        ...
        if ((forArr || notForArr) && $inputs && $inputs.length > 0) {
          $inputs.each(function () {
            ...
            if (inputName && self._fields[inputName]) {
              if (forArr) self._fields[inputName]._for = forArr;
              if (notForArr) self._fields[inputName]._notFor = notForArr;
            }
          });
        }
      });
    ...
    for (var fName in this._fields) {
      ...
      if (this._fields[fName]._for || this._fields[fName]._notFor) {
        if ((this._fields[fName]._for && !toeInArrayCfs(htmlCode, this._fields[fName]._for)) || (this._fields[fName]._notFor && toeInArrayCfs(htmlCode, this._fields[fName]._notFor))) {
          usable = false;
        }
      }
      if (usable) {
  ...
  _listOptsToArr: function (field) {
    ...
    if (field && field.options) {
      // This will be triggered when we add field - from it's DB settings - where it is already as array
      options = field.options;
    } else {
      // This will be triggered when we add it from edit form
      for (var key in field) {
  ...
  _initShellActions: function ($shell, data) {
    ...
    $shell.click(function () {
      if (self._sortInProgress) {
        // Sorting was just stopped - this is not a click
        self._sortInProgress = false;
        return false;
      }
  ...
  saveGoogleMapFieldWnd: function () {
    ...
    if (mapId) {
      var mapLabel = jQuery(this._googleMapsId)
        .find('option[value="' + mapId + '"]')
        .text();
      this.storeField({
  ...
  validateFieldData: function (fieldData) {
    ...
    if (!fieldData.name || (!nameRegExp.test(fieldData.name) && !toeInArrayCfs(fieldData.html, ['wooattrs']))) {
      errors.push('name');
    }
    if ((!fieldData.label || fieldData.label == '') && (!fieldData.placeholder || fieldData.placeholder == '') && !toeInArrayCfs(fieldData.html, ['recaptcha'])) {
...
jQuery(document).ready(function () {
  ...
  if (typeof cfsForm !== 'undefined' && cfsForm.params && cfsForm.params.fields && cfsForm.params.fields.length) {
...
function cfsGetFaIconsList() {
  return {
    glass: ['martini', 'drink', 'bar', 'alcohol', 'liquor', 'glass'],
    music: ['note', 'sound', 'music'],
    search: ['magnify', 'zoom', 'enlarge', 'bigger', 'search'],
    'envelope-o': ['email', 'support', 'e-mail', 'letter', 'mail', 'notification', 'envelope outlined'],
    heart: ['love', 'like', 'favorite', 'heart'],
    star: ['award', 'achievement', 'night', 'rating', 'score', 'favorite', 'star'],
    'star-o': ['award', 'achievement', 'night', 'rating', 'score', 'favorite', 'star outlined'],
    user: ['person', 'man', 'head', 'profile', 'user'],
    film: ['movie', 'film'],
    'th-large': ['blocks', 'squares', 'boxes', 'grid', 'th-large'],
    th: ['blocks', 'squares', 'boxes', 'grid', 'th'],
    'th-list': ['ul', 'ol', 'checklist', 'finished', 'completed', 'done', 'todo', 'th-list'],
    check: ['checkmark', 'done', 'todo', 'agree', 'accept', 'confirm', 'tick', 'ok', 'check'],
    times: ['close', 'exit', 'x', 'cross', 'times'],
    'search-plus': ['magnify', 'zoom', 'enlarge', 'bigger', 'search plus'],
    'search-minus': ['magnify', 'minify', 'zoom', 'smaller', 'search minus'],
    'power-off': ['on', 'power off'],
    signal: ['graph', 'bars', 'signal'],
    cog: ['settings', 'cog'],
    'trash-o': ['garbage', 'delete', 'remove', 'trash', 'hide', 'trash outlined'],
    home: ['main', 'house', 'home'],
    'file-o': ['new', 'page', 'pdf', 'document', 'file outlined'],
    'clock-o': ['watch', 'timer', 'late', 'timestamp', 'clock outlined'],
    road: ['street', 'road'],
    download: ['import', 'download'],
    'arrow-circle-o-down': ['download', 'arrow circle outlined down'],
    'arrow-circle-o-up': ['arrow circle outlined up'],
    inbox: ['inbox'],
    'play-circle-o': ['play circle outlined'],
    repeat: ['redo', 'forward', 'repeat'],
    refresh: ['reload', 'sync', 'refresh'],
    'list-alt': ['ul', 'ol', 'checklist', 'finished', 'completed', 'done', 'todo', 'list-alt'],
    lock: ['protect', 'admin', 'security', 'lock'],
    flag: ['report', 'notification', 'notify', 'flag'],
    headphones: ['sound', 'listen', 'music', 'audio', 'headphones'],
    'volume-off': ['audio', 'mute', 'sound', 'music', 'volume-off'],
    'volume-down': ['audio', 'lower', 'quieter', 'sound', 'music', 'volume-down'],
    'volume-up': ['audio', 'higher', 'louder', 'sound', 'music', 'volume-up'],
    qrcode: ['scan', 'qrcode'],
    barcode: ['scan', 'barcode'],
    tag: ['label', 'tag'],
    tags: ['labels', 'tags'],
    book: ['read', 'documentation', 'book'],
    bookmark: ['save', 'bookmark'],
    print: ['print'],
    camera: ['photo', 'picture', 'record', 'camera'],
    font: ['text', 'font'],
    bold: ['bold'],
    ...
    'align-center': ['middle', 'text', 'align-center'],
    'align-right': ['text', 'align-right'],
    'align-justify': ['text', 'align-justify'],
    list: ['ul', 'ol', 'checklist', 'finished', 'completed', 'done', 'todo', 'list'],
    outdent: ['outdent'],
    indent: ['indent'],
    'video-camera': ['film', 'movie', 'record', 'video camera'],
    'picture-o': ['picture outlined'],
    pencil: ['write', 'edit', 'update', 'pencil'],
    'map-marker': ['map', 'pin', 'location', 'coordinates', 'localize', 'address', 'travel', 'where', 'place', 'map-marker'],
    adjust: ['contrast', 'adjust'],
    tint: ['raindrop', 'waterdrop', 'drop', 'droplet', 'tint'],
    'pencil-square-o': ['write', 'edit', 'update', 'pencil square outlined'],
    'share-square-o': ['social', 'send', 'arrow', 'share square outlined'],
    'check-square-o': ['todo', 'done', 'agree', 'accept', 'confirm', 'ok', 'check square outlined'],
    arrows: ['move', 'reorder', 'resize', 'arrows'],
    'step-backward': ['rewind', 'previous', 'beginning', 'start', 'first', 'step-backward'],
    'fast-backward': ['rewind', 'previous', 'beginning', 'start', 'first', 'fast-backward'],
    backward: ['rewind', 'previous', 'backward'],
    play: ['start', 'playing', 'music', 'sound', 'play'],
    pause: ['wait', 'pause'],
    stop: ['block', 'box', 'square', 'stop'],
    forward: ['forward', 'next', 'forward'],
    'fast-forward': ['next', 'end', 'last', 'fast-forward'],
    'step-forward': ['next', 'end', 'last', 'step-forward'],
    eject: ['eject'],
    'chevron-left': ['bracket', 'previous', 'back', 'chevron-left'],
    'chevron-right': ['bracket', 'next', 'forward', 'chevron-right'],
    'plus-circle': ['add', 'new', 'create', 'expand', 'plus circle'],
    'minus-circle': ['delete', 'remove', 'trash', 'hide', 'minus circle'],
    'times-circle': ['close', 'exit', 'x', 'times circle'],
    'check-circle': ['todo', 'done', 'agree', 'accept', 'confirm', 'ok', 'check circle'],
    'question-circle': ['help', 'information', 'unknown', 'support', 'question circle'],
    'info-circle': ['help', 'information', 'more', 'details', 'info circle'],
    crosshairs: ['picker', 'crosshairs'],
    'times-circle-o': ['close', 'exit', 'x', 'times circle outlined'],
    'check-circle-o': ['todo', 'done', 'agree', 'accept', 'confirm', 'ok', 'check circle outlined'],
    ban: ['delete', 'remove', 'trash', 'hide', 'block', 'stop', 'abort', 'cancel', 'ban'],
    'arrow-left': ['previous', 'back', 'arrow-left'],
    'arrow-right': ['next', 'forward', 'arrow-right'],
    'arrow-up': ['arrow-up'],
    'arrow-down': ['download', 'arrow-down'],
    share: ['share'],
    expand: ['enlarge', 'bigger', 'resize', 'expand'],
    compress: ['collapse', 'combine', 'contract', 'merge', 'smaller', 'compress'],
    plus: ['add', 'new', 'create', 'expand', 'plus'],
    minus: ['hide', 'minify', 'delete', 'remove', 'trash', 'hide', 'collapse', 'minus'],
    asterisk: ['details', 'asterisk'],
    'exclamation-circle': ['warning', 'error', 'problem', 'notification', 'alert', 'exclamation circle'],
    gift: ['present', 'gift'],
    leaf: ['eco', 'nature', 'plant', 'leaf'],
    fire: ['flame', 'hot', 'popular', 'fire'],
    eye: ['show', 'visible', 'views', 'eye'],
    'eye-slash': ['toggle', 'show', 'hide', 'visible', 'visiblity', 'views', 'eye slash'],
    'exclamation-triangle': ['warning', 'error', 'problem', 'notification', 'alert', 'exclamation triangle'],
    plane: ['travel', 'trip', 'location', 'destination', 'airplane', 'fly', 'mode', 'plane'],
    calendar: ['date', 'time', 'when', 'event', 'calendar'],
    random: ['sort', 'shuffle', 'random'],
    comment: ['speech', 'notification', 'note', 'chat', 'bubble', 'feedback', 'message', 'texting', 'sms', 'conversation', 'comment'],
    magnet: ['magnet'],
    'chevron-up': ['chevron-up'],
    'chevron-down': ['chevron-down'],
    retweet: ['refresh', 'reload', 'share', 'retweet'],
    'shopping-cart': ['checkout', 'buy', 'purchase', 'payment', 'shopping-cart'],
    folder: ['folder'],
    'folder-open': ['folder open'],
    'arrows-v': ['resize', 'arrows vertical'],
    'arrows-h': ['resize', 'arrows horizontal'],
    'bar-chart': ['graph', 'analytics', 'bar chart'],
    'twitter-square': ['tweet', 'social network', 'twitter square'],
    'facebook-square': ['social network', 'facebook square'],
    'camera-retro': ['photo', 'picture', 'record', 'camera-retro'],
    key: ['unlock', 'password', 'key'],
    cogs: ['settings', 'cogs'],
    comments: ['speech', 'notification', 'note', 'chat', 'bubble', 'feedback', 'message', 'texting', 'sms', 'conversation', 'comments'],
    'thumbs-o-up': ['like', 'approve', 'favorite', 'agree', 'hand', 'thumbs up outlined'],
    'thumbs-o-down': ['dislike', 'disapprove', 'disagree', 'hand', 'thumbs down outlined'],
    'star-half': ['award', 'achievement', 'rating', 'score', 'star-half'],
    'heart-o': ['love', 'like', 'favorite', 'heart outlined'],
    'sign-out': ['log out', 'logout', 'leave', 'exit', 'arrow', 'sign out'],
    'linkedin-square': ['linkedin square'],
    'thumb-tack': ['marker', 'pin', 'location', 'coordinates', 'thumb tack'],
    'external-link': ['open', 'new', 'external link'],
    'sign-in': ['enter', 'join', 'log in', 'login', 'sign up', 'sign in', 'signin', 'signup', 'arrow', 'sign in'],
    trophy: ['award', 'achievement', 'cup', 'winner', 'game', 'trophy'],
    'github-square': ['octocat', 'github square'],
    upload: ['import', 'upload'],
    'lemon-o': ['food', 'lemon outlined'],
    phone: ['call', 'voice', 'number', 'support', 'earphone', 'telephone', 'phone'],
    'square-o': ['block', 'square', 'box', 'square outlined'],
    'bookmark-o': ['save', 'bookmark outlined'],
    'phone-square': ['call', 'voice', 'number', 'support', 'telephone', 'phone square'],
    twitter: ['tweet', 'social network', 'twitter'],
    facebook: ['social network', 'facebook'],
    github: ['octocat', 'github'],
    unlock: ['protect', 'admin', 'password', 'lock', 'unlock'],
    'credit-card': ['money', 'buy', 'debit', 'checkout', 'purchase', 'payment', 'credit-card'],
    rss: ['blog', 'rss'],
    'hdd-o': ['harddrive', 'hard drive', 'storage', 'save', 'hdd'],
    bullhorn: ['announcement', 'share', 'broadcast', 'louder', 'megaphone', 'bullhorn'],
    bell: ['alert', 'reminder', 'notification', 'bell'],
    certificate: ['badge', 'star', 'certificate'],
    'hand-o-right': ['point', 'right', 'next', 'forward', 'finger', 'hand outlined right'],
    'hand-o-left': ['point', 'left', 'previous', 'back', 'finger', 'hand outlined left'],
    'hand-o-up': ['point', 'finger', 'hand outlined up'],
    'hand-o-down': ['point', 'finger', 'hand outlined down'],
    'arrow-circle-left': ['previous', 'back', 'arrow circle left'],
    'arrow-circle-right': ['next', 'forward', 'arrow circle right'],
    'arrow-circle-up': ['arrow circle up'],
    'arrow-circle-down': ['download', 'arrow circle down'],
    globe: ['world', 'planet', 'map', 'place', 'travel', 'earth', 'global', 'translate', 'all', 'language', 'localize', 'location', 'coordinates', 'country', 'globe'],
    wrench: ['settings', 'fix', 'update', 'spanner', 'wrench'],
    tasks: ['progress', 'loading', 'downloading', 'downloads', 'settings', 'tasks'],
    filter: ['funnel', 'options', 'filter'],
    briefcase: ['work', 'business', 'office', 'luggage', 'bag', 'briefcase'],
    'arrows-alt': ['expand', 'enlarge', 'fullscreen', 'bigger', 'move', 'reorder', 'resize', 'arrow', 'arrows alt'],
    users: ['people', 'profiles', 'persons', 'users'],
    link: ['chain', 'link'],
    cloud: ['save', 'cloud'],
    flask: ['science', 'beaker', 'experimental', 'labs', 'flask'],
    scissors: ['scissors'],
    'files-o': ['duplicate', 'clone', 'copy', 'files outlined'],
    paperclip: ['attachment', 'paperclip'],
    'floppy-o': ['floppy outlined'],
    square: ['block', 'box', 'square'],
    bars: ['menu', 'drag', 'reorder', 'settings', 'list', 'ul', 'ol', 'checklist', 'todo', 'list', 'hamburger', 'bars'],
    'list-ul': ['ul', 'ol', 'checklist', 'todo', 'list', 'list-ul'],
    'list-ol': ['ul', 'ol', 'checklist', 'list', 'todo', 'list', 'numbers', 'list-ol'],
    strikethrough: ['strikethrough'],
    underline: ['underline'],
    table: ['data', 'excel', 'spreadsheet', 'table'],
    magic: ['wizard', 'automatic', 'autocomplete', 'magic'],
    truck: ['shipping', 'truck'],
    pinterest: ['pinterest'],
    'pinterest-square': ['pinterest square'],
    'google-plus-square': ['social network', 'google plus square'],
    'google-plus': ['social network', 'google plus'],
    money: ['cash', 'money', 'buy', 'checkout', 'purchase', 'payment', 'money'],
    'caret-down': ['more', 'dropdown', 'menu', 'triangle down', 'arrow', 'caret down'],
    'caret-up': ['triangle up', 'arrow', 'caret up'],
    'caret-left': ['previous', 'back', 'triangle left', 'arrow', 'caret left'],
    'caret-right': ['next', 'forward', 'triangle right', 'arrow', 'caret right'],
    columns: ['split', 'panes', 'columns'],
    sort: ['order', 'sort'],
    'sort-desc': ['dropdown', 'more', 'menu', 'arrow', 'sort descending'],
    'sort-asc': ['arrow', 'sort ascending'],
    envelope: ['email', 'e-mail', 'letter', 'support', 'mail', 'notification', 'envelope'],
    linkedin: ['linkedin'],
    undo: ['back', 'undo'],
    gavel: ['judge', 'lawyer', 'opinion', 'gavel'],
    tachometer: ['speedometer', 'fast', 'tachometer'],
    'comment-o': ['speech', 'notification', 'note', 'chat', 'bubble', 'feedback', 'message', 'texting', 'sms', 'conversation', 'comment-o'],
    'comments-o': ['speech', 'notification', 'note', 'chat', 'bubble', 'feedback', 'message', 'texting', 'sms', 'conversation', 'comments-o'],
    bolt: ['lightning', 'weather', 'lightning bolt'],
    sitemap: ['directory', 'hierarchy', 'organization', 'sitemap'],
    umbrella: ['umbrella'],
    clipboard: ['copy', 'clipboard'],
    'lightbulb-o': ['idea', 'inspiration', 'lightbulb outlined'],
    exchange: ['transfer', 'arrows', 'arrow', 'exchange'],
    'cloud-download': ['import', 'cloud download'],
    'cloud-upload': ['import', 'cloud upload'],
    'user-md': ['doctor', 'profile', 'medical', 'nurse', 'user-md'],
    stethoscope: ['stethoscope'],
    suitcase: ['trip', 'luggage', 'travel', 'move', 'baggage', 'suitcase'],
    'bell-o': ['alert', 'reminder', 'notification', 'bell outlined'],
    coffee: ['morning', 'mug', 'breakfast', 'tea', 'drink', 'cafe', 'coffee'],
    cutlery: ['food', 'restaurant', 'spoon', 'knife', 'dinner', 'eat', 'cutlery'],
    'file-text-o': ['new', 'page', 'pdf', 'document', 'file text outlined'],
    'building-o': ['work', 'business', 'apartment', 'office', 'company', 'building outlined'],
    'hospital-o': ['building', 'hospital outlined'],
    ambulance: ['vehicle', 'support', 'help', 'ambulance'],
    medkit: ['first aid', 'firstaid', 'help', 'support', 'health', 'medkit'],
    'fighter-jet': ['fly', 'plane', 'airplane', 'quick', 'fast', 'travel', 'fighter-jet'],
    beer: ['alcohol', 'stein', 'drink', 'mug', 'bar', 'liquor', 'beer'],
    'h-square': ['hospital', 'hotel', 'h square'],
    'plus-square': ['add', 'new', 'create', 'expand', 'plus square'],
    'angle-double-left': ['laquo', 'quote', 'previous', 'back', 'arrows', 'angle double left'],
    'angle-double-right': ['raquo', 'quote', 'next', 'forward', 'arrows', 'angle double right'],
    'angle-double-up': ['arrows', 'angle double up'],
    'angle-double-down': ['arrows', 'angle double down'],
    'angle-left': ['previous', 'back', 'arrow', 'angle-left'],
    'angle-right': ['next', 'forward', 'arrow', 'angle-right'],
    'angle-up': ['arrow', 'angle-up'],
    'angle-down': ['arrow', 'angle-down'],
    desktop: ['monitor', 'screen', 'desktop', 'computer', 'demo', 'device', 'desktop'],
    laptop: ['demo', 'computer', 'device', 'laptop'],
    tablet: ['ipad', 'device', 'tablet'],
    mobile: ['cell phone', 'cellphone', 'text', 'call', 'iphone', 'number', 'telephone', 'mobile phone'],
    'circle-o': ['circle outlined'],
    'quote-left': ['quote-left'],
    'quote-right': ['quote-right'],
    spinner: ['loading', 'progress', 'spinner'],
    circle: ['dot', 'notification', 'circle'],
    reply: ['reply'],
    'github-alt': ['octocat', 'github alt'],
    'folder-o': ['folder outlined'],
    'folder-open-o': ['folder open outlined'],
    'smile-o': ['face', 'emoticon', 'happy', 'approve', 'satisfied', 'rating', 'smile outlined'],
    'frown-o': ['face', 'emoticon', 'sad', 'disapprove', 'rating', 'frown outlined'],
    'meh-o': ['face', 'emoticon', 'rating', 'neutral', 'meh outlined'],
    gamepad: ['controller', 'gamepad'],
    'keyboard-o': ['type', 'input', 'keyboard outlined'],
    'flag-o': ['report', 'notification', 'flag outlined'],
    'flag-checkered': ['report', 'notification', 'notify', 'flag-checkered'],
    terminal: ['command', 'prompt', 'code', 'terminal'],
    code: ['html', 'brackets', 'code'],
    'reply-all': ['reply-all'],
    'star-half-o': ['award', 'achievement', 'rating', 'score', 'star half outlined'],
    'location-arrow': ['map', 'coordinates', 'location', 'address', 'place', 'where', 'location-arrow'],
    crop: ['crop'],
    'code-fork': ['git', 'fork', 'vcs', 'svn', 'github', 'rebase', 'version', 'merge', 'code-fork'],
    'chain-broken': ['remove', 'chain broken'],
    question: ['help', 'information', 'unknown', 'support', 'question'],
    info: ['help', 'information', 'more', 'details', 'info'],
    exclamation: ['warning', 'error', 'problem', 'notification', 'notify', 'alert', 'exclamation'],
    superscript: ['exponential', 'superscript'],
    subscript: ['subscript'],
    eraser: ['remove', 'delete', 'eraser'],
    'puzzle-piece': ['addon', 'add-on', 'section', 'puzzle piece'],
    microphone: ['record', 'voice', 'sound', 'microphone'],
    'microphone-slash': ['record', 'voice', 'sound', 'mute', 'microphone slash'],
    shield: ['award', 'achievement', 'security', 'winner', 'shield'],
    'calendar-o': ['date', 'time', 'when', 'event', 'calendar-o'],
    'fire-extinguisher': ['fire-extinguisher'],
    rocket: ['app', 'rocket'],
    maxcdn: ['maxcdn'],
    'chevron-circle-left': ['previous', 'back', 'arrow', 'chevron circle left'],
    'chevron-circle-right': ['next', 'forward', 'arrow', 'chevron circle right'],
    'chevron-circle-up': ['arrow', 'chevron circle up'],
    'chevron-circle-down': ['more', 'dropdown', 'menu', 'arrow', 'chevron circle down'],
    html5: ['html 5 logo'],
    css3: ['code', 'css 3 logo'],
    anchor: ['link', 'anchor'],
    'unlock-alt': ['protect', 'admin', 'password', 'lock', 'unlock alt'],
    bullseye: ['target', 'bullseye'],
    'ellipsis-h': ['dots', 'ellipsis horizontal'],
    'ellipsis-v': ['dots', 'ellipsis vertical'],
    'rss-square': ['feed', 'blog', 'rss square'],
    'play-circle': ['start', 'playing', 'play circle'],
    ticket: ['movie', 'pass', 'support', 'ticket'],
    'minus-square': ['hide', 'minify', 'delete', 'remove', 'trash', 'hide', 'collapse', 'minus square'],
    'minus-square-o': ['hide', 'minify', 'delete', 'remove', 'trash', 'hide', 'collapse', 'minus square outlined'],
    'level-up': ['arrow', 'level up'],
    'level-down': ['arrow', 'level down'],
    'check-square': ['checkmark', 'done', 'todo', 'agree', 'accept', 'confirm', 'ok', 'check square'],
    'pencil-square': ['write', 'edit', 'update', 'pencil square'],
    'external-link-square': ['open', 'new', 'external link square'],
    'share-square': ['social', 'send', 'share square'],
    compass: ['safari', 'directory', 'menu', 'location', 'compass'],
    'caret-square-o-down': ['more', 'dropdown', 'menu', 'caret square outlined down'],
    'caret-square-o-up': ['caret square outlined up'],
    'caret-square-o-right': ['next', 'forward', 'caret square outlined right'],
    eur: ['euro (eur)'],
    gbp: ['gbp'],
    usd: ['us dollar'],
    ...
    krw: ['korean won (krw)'],
    btc: ['bitcoin (btc)'],
    file: ['new', 'page', 'pdf', 'document', 'file'],
    'file-text': ['new', 'page', 'pdf', 'document', 'file text'],
    ...
    'sort-numeric-asc': ['numbers', 'sort numeric ascending'],
    'sort-numeric-desc': ['numbers', 'sort numeric descending'],
    'thumbs-up': ['like', 'favorite', 'approve', 'agree', 'hand', 'thumbs-up'],
    'thumbs-down': ['dislike', 'disapprove', 'disagree', 'hand', 'thumbs-down'],
    'youtube-square': ['video', 'film', 'youtube square'],
    youtube: ['video', 'film', 'youtube'],
    xing: ['xing'],
    'xing-square': ['xing square'],
    'youtube-play': ['start', 'playing', 'youtube play'],
    dropbox: ['dropbox'],
    ...
    'bitbucket-square': ['git', 'bitbucket square'],
    tumblr: ['tumblr'],
    'tumblr-square': ['tumblr square'],
    'long-arrow-down': ['long arrow down'],
    'long-arrow-up': ['long arrow up'],
    'long-arrow-left': ['previous', 'back', 'long arrow left'],
    'long-arrow-right': ['long arrow right'],
    apple: ['osx', 'food', 'apple'],
    ...
    trello: ['trello'],
    female: ['woman', 'user', 'person', 'profile', 'female'],
    male: ['man', 'user', 'person', 'profile', 'male'],
    gratipay: ['heart', 'like', 'favorite', 'love', 'gratipay (gittip)'],
    'sun-o': ['weather', 'contrast', 'lighter', 'brighten', 'day', 'sun outlined'],
    'moon-o': ['night', 'darker', 'contrast', 'moon outlined'],
    archive: ['box', 'storage', 'archive'],
    bug: ['report', 'insect', 'bug'],
    vk: ['vk'],
    weibo: ['weibo'],
    renren: ['renren'],
    pagelines: ['leaf', 'leaves', 'tree', 'plant', 'eco', 'nature', 'pagelines'],
    'stack-exchange': ['stack exchange'],
    'arrow-circle-o-right': ['next', 'forward', 'arrow circle outlined right'],
    'arrow-circle-o-left': ['previous', 'back', 'arrow circle outlined left'],
    'caret-square-o-left': ['previous', 'back', 'caret square outlined left'],
    'dot-circle-o': ['target', 'bullseye', 'notification', 'dot circle outlined'],
    wheelchair: ['handicap', 'person', 'wheelchair'],
    'vimeo-square': ['vimeo square'],
    try: ['turkish lira (try)'],
    'plus-square-o': ['add', 'new', 'create', 'expand', 'plus square outlined'],
    'space-shuttle': ['space shuttle'],
    slack: ['hashtag', 'anchor', 'hash', 'slack logo'],
    'envelope-square': ['envelope square'],
    wordpress: ['wordpress logo'],
    openid: ['openid'],
    university: ['university'],
    'graduation-cap': ['learning', 'school', 'student', 'graduation cap'],
    yahoo: ['yahoo logo'],
    google: ['google logo'],
    reddit: ['reddit logo'],
    ...
    language: ['language'],
    fax: ['fax'],
    building: ['work', 'business', 'apartment', 'office', 'company', 'building'],
    child: ['child'],
    paw: ['pet', 'paw'],
    spoon: ['spoon'],
    cube: ['cube'],
    ...
    plug: ['power', 'connect', 'plug'],
    slideshare: ['slideshare'],
    twitch: ['twitch'],
    yelp: ['yelp'],
    'newspaper-o': ['press', 'newspaper outlined'],
    wifi: ['wifi'],
    ...
    'cc-amex': ['amex', 'american express credit card'],
    'cc-paypal': ['paypal credit card'],
    'cc-stripe': ['stripe credit card'],
    'bell-slash': ['bell slash'],
    'bell-slash-o': ['bell slash outlined'],
    trash: ['garbage', 'delete', 'remove', 'hide', 'trash'],
    copyright: ['copyright'],
    at: ['at'],
    eyedropper: ['eyedropper'],
    'paint-brush': ['paint brush'],
    'birthday-cake': ['birthday cake'],
    'area-chart': ['graph', 'analytics', 'area chart'],
    'pie-chart': ['graph', 'analytics', 'pie chart'],
    'line-chart': ['graph', 'analytics', 'line chart'],
    lastfm: ['last.fm'],
    'lastfm-square': ['last.fm square'],
    'toggle-off': ['toggle off'],
    'toggle-on': ['toggle on'],
    bicycle: ['vehicle', 'bike', 'bicycle'],
    bus: ['vehicle', 'bus'],
    ioxhost: ['ioxhost'],
    angellist: ['angellist'],
    cc: ['closed captions'],
    ...
    skyatlas: ['skyatlas'],
    'cart-plus': ['add', 'shopping', 'add to shopping cart'],
    'cart-arrow-down': ['shopping', 'shopping cart arrow down'],
    diamond: ['gem', 'gemstone', 'diamond'],
    ship: ['boat', 'sea', 'ship'],
    'user-secret': ['whisper', 'spy', 'incognito', 'privacy', 'user secret'],
    motorcycle: ['vehicle', 'bike', 'motorcycle'],
    'street-view': ['map', 'street view'],
    heartbeat: ['ekg', 'heartbeat'],
    venus: ['female', 'venus'],
    mars: ['male', 'mars'],
    mercury: ['transgender', 'mercury'],
    ...
    whatsapp: ["what's app"],
    server: ['server'],
    'user-plus': ['sign up', 'signup', 'add user'],
    ...
    'battery-full': ['power', 'battery full'],
    'battery-three-quarters': ['power', 'battery 3/4 full'],
    'battery-half': ['power', 'battery 1/2 full'],
    'battery-quarter': ['power', 'battery 1/4 full'],
    'battery-empty': ['power', 'battery empty'],
    ...
    opera: ['opera'],
    'internet-explorer': ['browser', 'ie', 'internet-explorer'],
    television: ['display', 'computer', 'monitor', 'television'],
    contao: ['contao'],
    '500px': ['500px'],
    amazon: ['amazon'],
    ...
    'calendar-check-o': ['ok', 'calendar check outlined'],
    industry: ['factory', 'industry'],
    'map-pin': ['map pin'],
    'map-signs': ['map signs'],
    'map-o': ['map outlined'],
    map: ['map'],
    commenting: ['speech', 'notification', 'note', 'chat', 'bubble', 'feedback', 'message', 'texting', 'sms', 'conversation', 'commenting'],
    'commenting-o': ['speech', 'notification', 'note', 'chat', 'bubble', 'feedback', 'message', 'texting', 'sms', 'conversation', 'commenting outlined'],
    houzz: ['houzz'],
    vimeo: ['vimeo'],
    'black-tie': ['font awesome black tie'],
    fonticons: ['fonticons'],
    'reddit-alien': ['reddit alien'],
    edge: ['browser', 'ie', 'edge browser'],
    'credit-card-alt': ['money', 'buy', 'debit', 'checkout', 'purchase', 'payment', 'credit card', 'credit card'],
    codiepie: ['codie pie'],
    modx: ['modx'],
    ...
    wpforms: ['wpforms'],
    envira: ['leaf', 'envira gallery'],
    'universal-access': ['universal access'],
    'wheelchair-alt': ['handicap', 'person', 'wheelchair alt'],
    'question-circle-o': ['question circle outlined'],
    blind: ['blind'],
    'audio-description': ['audio description'],
    'volume-control-phone': ['telephone', 'volume control phone'],
    braille: ['braille'],
...
jQuery(document).on('click', '.fa', function () {
  if (jQuery(this).parent().hasClass('cfsFieldIconBtn')) {
    _cfsSelectFieldIconClb(jQuery(this).parent());
  }
});
var wait = false;
