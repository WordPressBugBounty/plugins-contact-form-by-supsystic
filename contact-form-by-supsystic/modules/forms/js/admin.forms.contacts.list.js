jQuery(document).ready(function () {
  ...
  jQuery('#' + tblId).jqGrid({
    ...
    onSelectRow: function (rowid, e, event) {
      ...
      if (selectedRowIds && selectedRowIds.length == 1 && jQuery(event.target).attr('aria-describedby') != tblId + '_cb') {
  ...
  jQuery('#cb_' + tblId + '').change(function () {
    jQuery(this).attr('checked') ? jQuery('#cfsFormContactsRemoveGroupBtn').removeAttr('disabled') : jQuery('#cfsFormContactsRemoveGroupBtn').attr('disabled', 'disabled');
  });
  jQuery('#cfsFormContactsRemoveGroupBtn').click(function () {
    ...
    var confirmMsg = listIds.length > 1 ? toeLangCfs('Are you sur want to remove ' + listIds.length + ' Form Contactss?') : toeLangCfs('Are you sure want to remove 1 Form Contact?');
          ...
          for (var i = 0; i < res.data.form_fields.length; i++) {
            ...
            if (typeof res.data.contact.fields[fieldName] !== 'undefined') {
              var $newRow = $rowEx.clone().removeAttr('id').appendTo($shell),
                fieldLabel = res.data.form_fields[i].label ? res.data.form_fields[i].label : res.data.form_fields[i].placeholder,
