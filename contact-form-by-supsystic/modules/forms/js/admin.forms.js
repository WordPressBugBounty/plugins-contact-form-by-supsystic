...
jQuery(document).ready(function () {
  if (typeof cfsOriginalForm !== 'undefined') {
    // Just changing template - for existing forms
    cfsInitChangeFormDialog();
  } else {
    // Creating new forms
    cfsInitCreateFormDialog();
  }
  if (jQuery('.cfsTplPrevImg').length) {
    // If on creation page
  ...
  jQuery('.forms-list-item').click(function () {
    ...
    if (cfsOriginalForm.original_id == id) {
      var dialog = jQuery('<div />')
        .html(toeLangCfs('This is the same template that was used for Form before.'))
        .dialog({
          modal: true,
          width: 480,
          height: 180,
...
function _cfsShowPromoFormForTpl($tplItem) {
  var $proOptWnd = cfsGetMainPromoForm(),
    selectedTplHref = $tplItem ? jQuery($tplItem).find('a.cfsPromoTplBtn').attr('href') : jQuery('.forms-list-item.active a.cfsPromoTplBtn').attr('href');
