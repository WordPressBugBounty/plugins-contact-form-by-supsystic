...
class formsModelCfs extends modelCfs
{
  ...
  {
    global $wpdb;
    $allForms = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND ab_id = 0", ARRAY_A);
    foreach ($allForms as $key => $forms) {
      $allForms[$key] = $this->_afterGetFromTbl($forms);
    }
    return $allForms;
  ...
  {
    global $wpdb;
    $res = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}cfs_forms WHERE id = %s", $id), ARRAY_A);
    if ($res) {
  ...
  public function contact($d)
  {
    ...
    if ($id) {
      ...
      $form = $this->supGetById($id);
      if ($form) {
        if ($this->validateFields($d['fields'], $form, $d)) {
          // If subscribe feature is available - it should go before sending any contact data
          if (isset($form['params']['tpl']['enb_subscribe']) && $form['params']['tpl']['enb_subscribe'] && frameCfs::_()->getModule('subscribe')) {
          ...
          // Publish here
          if (isset($form['params']['tpl']['enb_publish']) && $form['params']['tpl']['enb_publish'] && frameCfs::_()->getModule('publish')) {
          ...
          if ($this->sendContact($d['fields'], $form)) {
            // Registration
            if (isset($form['params']['tpl']['enb_reg']) && $form['params']['tpl']['enb_reg'] && frameCfs::_()->getModule('publish')) {
  ...
  public function saveContact($d, $form)
  {
    ...
    return $this->supInsert($saveData, 'contacts');
  }
  public function getContactsForForm($id)
  {
    $contacts = frameCfs::_()->getModule('forms')->getModel('contacts')->supGetByFormId($id);
    if (!empty($contacts)) {
  ...
  public function validateFields($fieldsData, $form, $d = [])
  {
    if (isset($form['params']['fields']) && !empty($form['params']['fields'])) {
      ...
      foreach ($form['params']['fields'] as $f) {
        ...
        $htmlType = $f['html'];
        $isDate = in_array($htmlType, ['date', 'month', 'week', 'time']);
        ...
        $formInvalidError = isset($form['params']['tpl']['field_error_invalid']) && !empty($form['params']['tpl']['field_error_invalid']) ? trim($form['params']['tpl']['field_error_invalid']) : false;
        if ($value) {
        ...
        if (empty($errorMsg)) {
          ...
          if (
            ...
            !in_array($htmlType, ['file']) // We already checked files size when we uploaded it - in add_fieldsModel::uploadFile()
          ) {
            $isNumber = $htmlType == 'number' && $value && is_numeric($value);
            if (($isNumber && $value > $maxSize) || (!$isNumber && $value && is_string($value) && strlen($value) > $maxSize)) {
              $errorMsg = sprintf(__('Maximum value for %s is %d', CFS_LANG_CODE), '%s', $maxSize);
            }
          }
          if (isset($f['vn_pattern']) && !empty($f['vn_pattern']) && !in_array($htmlType, ['file'])) {
        ...
        if (empty($errors[$k])) {
          // Additional check in pro module
  ...
  public function sendContact($fieldsData, $form)
  {
    if (isset($form['params']['submit']) && !empty($form['params']['submit'])) {
      ...
      $sendFormData = $this->_generateSendFormData($fieldsData, $form);
      $sendFormDataStr = $this->_generateFormDataStr($sendFormData, $form);
      $variables = $variablesWithSelectVals = [
        'sitename' => $blogName,
        'siteurl' => $siteUrl,
      ];
      foreach ($form['params']['fields'] as $f) {
      ...
      foreach ($form['params']['submit'] as $s) {
        ...
        if (empty($to)) {
          continue;
        } // Any reasn to send to empty email
        $msg = trim($s['msg']);
        $additionalHeaders = [];
        ...
        $msg = utilsCfs::replaceVariables($msg, array_merge($variables, ['form_data' => $sendFormDataStr]));
        $msg = str_replace(['</p><br />', '</p><br/>', '</p><br>'], '</p>', $msg);
        ...
        $sendRes = frameCfs::_()->getModule('mail')->send($to, $subject, $msg, $blogName, $adminEmail, $blogName, $adminEmail, $additionalHeaders);
        if (!$sendRes) {
  ...
  private function _generateSendFormData($fieldsData, $form)
  {
    ...
    foreach ($form['params']['fields'] as $f) {
      ...
      } elseif (in_array($htmlType, ['checkbox', 'radiobutton'])) {
        if ($value === false) {
          $sendValue = __('No', CFS_LANG_CODE);
        } else {
          $sendValue = empty($value) ? __('Yes', CFS_LANG_CODE) : $value;
        }
      } elseif (in_array($htmlType, ['checkboxlist', 'selectlist'])) {
        if ($value) {
          $sendValue = [];
      ...
      } elseif (in_array($htmlType, ['countryListMultiple'])) {
        if ($value) {
          $sendValue = [];
      ...
      } elseif (in_array($htmlType, ['countryList'])) {
        if ($value) {
          $options = fieldAdapterCfs::getCountries();
          $sendValue = $options[$value];
        }
      } elseif (in_array($htmlType, ['radiobuttons', 'selectbox'])) {
        if ($value) {
          $options = dbCfs::prepareHtmlIn($f['options']);
          foreach ($options as $fOpt) {
            if ($fOpt['name'] == $value) {
    ...
    if (!$dsblHtmlEmail && $maxCols > 1 && $emailAsnForm) {
      ...
      foreach ($form['params']['fields'] as $f) {
        ...
        if (isset($resNamed[$name])) {
          ...
          if (!$added) {
            // New row
            $rows[] = ['id' => $bsClassId, 'cols' => [$resNamed[$name]]];
          }
        }
      }
      foreach ($rows as $r) {
   ...
   */
  public function getSimpleList($where = [], $params = [])
  {
    if ($where) {
      $this->setWhere($where);
    }
  ...
  private function _getLinksReplacement()
  {
    if (empty($this->_linksReplacement)) {
      $this->_linksReplacement = [
        'modUrl' => ['url' => $this->getModule()->getModPath(), 'key' => 'CFS_MOD_URL'],
        'siteUrl' => ['url' => CFS_SITE_URL, 'key' => 'CFS_SITE_URL'],
        'assetsUrl' => ['url' => $this->getModule()->getAssetsUrl(), 'key' => 'CFS_ASSETS_URL'],
      ];
    }
  ...
  {
    global $wpdb;
    if (class_exists('ab_testingCfs')) {
      $abBaseId = frameCfs::_()->getModule('ab_testing')->getListForBaseId();
    }
    if (!empty($abBaseId)) {
      $abTestCondAdded = true;
      if (!empty($search)) {
        $count = (int) $wpdb->get_var("SELECT COUNT(*) AS total FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND " . $wpdb->prepare('ab_id = %s AND (id = %s OR label = %s OR views = %s OR unique_views = %s)', $abBaseId, $search, $search, $search, $search));
      } else {
        $count = (int) $wpdb->get_var("SELECT COUNT(*) AS total FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND " . $wpdb->prepare('ab_id = %s', $abBaseId));
      }
    } else {
      if (!empty($search)) {
        $count = (int) $wpdb->get_var("SELECT COUNT(*) AS total FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND ab_id = 0 AND " . $wpdb->prepare('(id = %s OR label = %s OR views = %s OR unique_views = %s)', $search, $search, $search, $search));
      } else {
        $count = (int) $wpdb->get_var("SELECT COUNT(*) AS total FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND ab_id = 0");
      }
    }
    return $count;
  }
    ...
    if (!empty($abBaseId)) {
      ...
      if (!empty($search)) {
        $data = $wpdb->get_results(
          "SELECT id, label, views, unique_views, actions, active FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND " .
            $wpdb->prepare(' ab_id = %s AND (id = %s OR label = %s OR views = %s OR unique_views = %s) ORDER BY id ASC LIMIT %1s,%1s', $abBaseId, $search, $search, $search, $search, (int) $limitStart, (int) $rowsLimit),
          ARRAY_A,
        );
      } else {
        $data = $wpdb->get_results("SELECT id, label, views, unique_views, actions, active FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND " . $wpdb->prepare(' ab_id = %s ORDER BY id ASC LIMIT %1s,%1s', $abBaseId, (int) $limitStart, (int) $rowsLimit), ARRAY_A);
      }
    } else {
      if (!empty($search)) {
        $data = $wpdb->get_results(
          "SELECT id, label, views, unique_views, actions, active FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND ab_id = 0 AND " .
            $wpdb->prepare(' (id = %s OR label = %s OR views = %s OR unique_views = %s) ORDER BY id ASC LIMIT %1s,%1s', $search, $search, $search, $search, (int) $limitStart, (int) $rowsLimit),
          ARRAY_A,
        );
      } else {
        $data = $wpdb->get_results("SELECT id, label, views, unique_views, actions, active FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND ab_id = 0 " . $wpdb->prepare(' ORDER BY id ASC LIMIT %1s,%1s', (int) $limitStart, (int) $rowsLimit), ARRAY_A);
      }
    }
    return $data;
  }
  public function getContactsTotalCountBySearch($search)
  {
    global $wpdb;
    $count = (int) $wpdb->get_var("SELECT COUNT(*) AS total FROM {$wpdb->prefix}cfs_contacts WHERE form_id != 0 " . $wpdb->prepare('OR id = %s', $search));
    return $count;
  }
  public function getContactsListForTblBySearch($search, $limitStart, $rowsLimit, $formId)
  {
    ...
    if (!empty($search)) {
      if (!empty($formId)) {
        $data = $wpdb->get_results("SELECT id, form_id, fields, ip, url, date_created FROM {$wpdb->prefix}cfs_contacts WHERE " . $wpdb->prepare('form_id = %s AND id = %s OR ip = %s ORDER BY id ASC LIMIT %1s,%1s', $formId, $search, $search, (int) $limitStart, (int) $rowsLimit), ARRAY_A);
      } else {
        $data = $wpdb->get_results(
          "SELECT id, form_id, fields, ip, url, date_created FROM {$wpdb->prefix}cfs_contacts WHERE form_id != 0 AND " . $wpdb->prepare('id = %s OR ip = %s OR form_id = %s ORDER BY id ASC LIMIT %1s,%1s', $search, $search, $search, (int) $limitStart, (int) $rowsLimit),
          ARRAY_A,
        );
      }
    }
    if (empty($search)) {
      if (!empty($formId)) {
        $data = $wpdb->get_results("SELECT id, form_id, fields, ip, url, date_created FROM {$wpdb->prefix}cfs_contacts WHERE form_id != 0 " . $wpdb->prepare(' AND form_id = %s ORDER BY id ASC LIMIT %1s,%1s', $formId, (int) $limitStart, (int) $rowsLimit), ARRAY_A);
      } else {
        $data = $wpdb->get_results("SELECT id, form_id, fields, ip, url, date_created FROM {$wpdb->prefix}cfs_contacts WHERE form_id != 0 " . $wpdb->prepare('ORDER BY id ASC LIMIT %1s,%1s', (int) $limitStart, (int) $rowsLimit), ARRAY_A);
      }
    }
    return $data;
  }
  public function getFormsTplList()
  {
    global $wpdb;
    $tplList = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}cfs_forms WHERE active = 1 AND original_id = 0 ORDER BY sort_order ASC", ARRAY_A);
    foreach ($tplList as $key => $row) {
      $tplList[$key] = $this->_afterGetFromTbl($row);
    }
    return $tplList;
  }
  ...
  public function insertFromOriginal($original)
  {
    ...
    $original = $this->_dataSave($original);
    return $this->supInsert($original);
  }
  ...
  public function clear()
  {
    if (
      frameCfs::_()
        ->getTable($this->_tbl)
        ->delete(['additionalCondition' => 'original_id != 0'])
    ) {
      return true;
    } else {
      $this->pushError(__('Database error detected', CFS_LANG_CODE));
  ...
  public function save($d = [])
  {
    ...
    if (isset($d['params']['tpl']['use_sss_prj_id'])) {
      ...
      if ($oldSssProjId != $newSssProjId) {
        if (!$this->_updateSocSharingProject($newSssProjId, $d['id'])) {
          // For just changed Proj ID - set it, if it was set to 0 - clear prev. selected
    ...
    if (isset($d['params']['enableForMembership'])) {
      ...
      if ($membershipModel) {
        $membershipRes = $membershipModel->updateRow(['form_id' => $d['id'], 'allow_use' => $d['params']['enableForMembership']]);
      }
    }
  ...
  {
    foreach ($d as $k => $v) {
      if (!in_array($k, ['id', 'params'])) {
        unset($d[$k]);
      }
    }
    return $this->updateById($d);
  }
  public function changeTpl($d = [])
  {
    $d['id'] = isset($d['id']) ? (int) $d['id'] : 0;
    $d['new_tpl_id'] = isset($d['new_tpl_id']) ? (int) $d['new_tpl_id'] : 0;
    if ($d['id'] && $d['new_tpl_id']) {
      $currentForm = $this->supGetById($d['id']);
      $newTpl = $this->supGetById($d['new_tpl_id']);
      $originalForm = $this->supGetById($currentForm['original_id']);
      $diffFromOriginal = $this->getDifferences($currentForm, $originalForm);
      if (!empty($diffFromOriginal)) {
        if (isset($newTpl['params'])) {
          $keysForMove = ['params.fields', 'params.submit', 'params.tpl.label', 'params.tpl.anim_key', 'params.tpl.enb_foot_note', 'params.tpl.foot_note', 'params.tpl.enb_sm', 'params.tpl.enb_subscribe'];
          foreach ($diffFromOriginal as $k) {
            if (in_array($k, $keysForMove) || strpos($k, 'params.tpl.enb_sm_') === 0 || strpos($k, 'params.tpl.sm_') === 0 || strpos($k, 'params.tpl.enb_sub_') === 0 || strpos($k, 'params.tpl.sub_') === 0 || strpos($k, 'params.tpl.enb_txt_') === 0 || strpos($k, 'params.tpl.txt_') === 0) {
      ...
      return $this->update($newTpl, ['id' => $newTpl['id']]);
    } else {
  ...
  private function _computeDifferences($forms, $original, $key = '', $keysImplode = [])
  {
    $difs = [];
    $checkAsArray = false;
    if (!empty($key)) {
      $checkAsArrayKeys = ['params.fields', 'params.submit'];
    ...
    if (is_array($forms) && !$checkAsArray) {
      $excludeKey = ['id', 'label', 'active', 'original_id', 'img_preview', 'date_created', 'view_id', 'view_html_id', 'actions', 'unique_views', 'views', 'img_preview_url', 'show_on', 'show_to', 'show_pages'];
      if (!empty($key)) {
        $keysImplode[] = $key;
      }
      foreach ($forms as $k => $v) {
        if (in_array($k, $excludeKey) && empty($key)) {
          continue;
        }
        if (!isset($original[$k])) {
  ...
  public function clearCachedStats($id)
  {
    $tbl = $this->getTbl();
    $id = (int) $id;
    $data = ['views' => 0, 'unique_views' => 0, 'actions' => 0];
  ...
  public function addCachedStat($id, $statColumn)
  {
    ...
    switch ($statColumn) {
      case 'views':
        $result = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}cfs_forms SET views = views + 1 WHERE id = %d", $id));
        break;
      case 'unique_views':
        $result = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}cfs_forms SET unique_views = unique_views + 1 WHERE id = %d", $id));
        break;
      case 'actions':
        $result = $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}cfs_forms SET actions = actions + 1 WHERE id = %d", $id));
        break;
    }
    return false;
  }
  ...
  public function saveAsCopy($d = [])
  {
    $d['copy_label'] = isset($d['copy_label']) ? sanitize_text_field(trim($d['copy_label'])) : '';
    $d['id'] = isset($d['id']) ? (int) $d['id'] : 0;
  ...
  public function switchActive($d = [])
  {
