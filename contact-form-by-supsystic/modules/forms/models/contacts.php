...
class contactsModelCfs extends modelCfs
{
  ...
  {
    global $wpdb;
    $res = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}cfs_contacts WHERE id = %s", $id), ARRAY_A);
    if ($res) {
  ...
  {
    global $wpdb;
    $res = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}cfs_contacts WHERE form_id = %s", $id), ARRAY_A);
    return $res;
  }
