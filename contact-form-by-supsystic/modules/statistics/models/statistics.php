...
class statisticsModelCfs extends modelCfs
{
  ...
  public function add($d = [])
  {
    ...
    if (!empty($d['id']) && !empty($d['type'])) {
      ...
      if (in_array($d['type'], ['show'])) {
        ...
        if (!in_array($d['type'], ['submit', 'submit_error'])) {
          // Do not count empty submits here
   ...
   */
  public function getForForm($formId, $params = [])
  {
      ...
      case 'week':
        if ($formId) {
          $count = $wpdb->get_results(
            "SELECT COUNT(*) AS total_requests, SUM(is_unique) AS unique_requests, DATE_FORMAT(DATE_SUB(date_created, INTERVAL DAYOFWEEK(date_created)-1 DAY), '%m-%d-%Y') AS date FROM {$wpdb->prefix}cfs_statistics WHERE " . $wpdb->prepare(' form_id = %s ORDER BY date DESC', $formId),
            ARRAY_A,
          );
        }
        if ($formId && $typeId) {
          $count = $wpdb->get_results(
            "SELECT COUNT(*) AS total_requests, SUM(is_unique) AS unique_requests, DATE_FORMAT(DATE_SUB(date_created, INTERVAL DAYOFWEEK(date_created)-1 DAY), '%m-%d-%Y') AS date FROM {$wpdb->prefix}cfs_statistics WHERE " .
              $wpdb->prepare(' form_id = %s AND type = %s ORDER BY date DESC', $formId, $typeId),
            ARRAY_A,
          );
        }
        break;
  ...
  public function getAllForFormSorted($id, $params = [])
  {
  ...
  public function getStats($d)
  {
    ...
    switch ($chartId) {
      case 'cfsMainStats':
        $stats = $this->getAllForFormSorted($id, ['group' => $group]);
        return $stats;
      case 'cfsRatingStats':
        return frameCfs::_()->getModule('add_fields') && frameCfs::_()->getModule('add_fields')->getModel('rating')
          ? frameCfs::_()
            ->getModule('add_fields')
            ->getModel('rating')
            ->getStatsForFormSorted($id, ['group' => $group])
          : [];
    }
    return false;
  }
  public function clearForForm($d = [])
  {
    $d['id'] = isset($d['id']) ? (int) $d['id'] : 0;
    if ($d['id']) {
      frameCfs::_()->getModule('forms')->getModel()->clearCachedStats($d['id']);
      return $this->delete(['form_id' => $d['id']]);
    } else {
      $this->pushError(__('Invalid ID', CFS_LANG_CODE));
    }
    return false;
  }
  public function getAllForFormId($id, $params = [])
  {
    $allTypes = $this->getModule()->getTypes();
    $allStats = [];
    $haveData = false;
  ...
  public function getUpdatedStats($d = [])
  {
  ...
  public function getPreparedStats($d = [])
  {
      ...
      foreach ($stats as $i => $stat) {
        if (isset($stat['points']) && !empty($stat['points'])) {
          foreach ($stat['points'] as $j => $point) {
            ...
            } elseif (!in_array($stat['code'], ['submit', 'submit_error'])) {
              // Do not count empty submits here
