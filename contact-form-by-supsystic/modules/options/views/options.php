<?php
class optionsViewCfs extends viewCfs
{
  private $_news = [];
  public function getNewFeatures()
  {
    $res = [];
    $readmePath = CFS_DIR . 'readme.txt';
    if (file_exists($readmePath)) {
      $readmeContent = @file_get_contents($readmePath);
      if (!empty($readmeContent)) {
        $matchedData = '';
        if (preg_match('/= ' . CFS_VERSION . ' =(.+)=.+=/isU', $readmeContent, $matches)) {
          $matchedData = $matches[1];
        } elseif (preg_match('/= ' . CFS_VERSION . ' =(.+)/is', $readmeContent, $matches)) {
          $matchedData = $matches[1];
        }
  ...
  public function getAdminPage()
  {
    ...
    if (isset($tabs[$activeTab]) && isset($tabs[$activeTab]['callback'])) {
      //frameCfs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
    ...
    frameCfs::_()->addJSVar('adminOptionsCfs', 'cfsActiveTab', $activeTab);
    ...
    $this->assign('breadcrumbs', frameCfs::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
  ...
  {
    frameCfs::_()->addScript('admin.mainoptions', $this->getModule()->getModPath() . 'js/admin.mainoptions.js');
  ...
  {
    frameCfs::_()->addScript('admin.settings', $this->getModule()->getModPath() . 'js/admin.settings.js');
    frameCfs::_()->getModule('templates')->loadJqueryUi();

    $options = frameCfs::_()->getModule('options')->getAll();
