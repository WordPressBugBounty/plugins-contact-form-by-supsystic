            ...
            <nav class="supsystic-navigation supsystic-sticky <?php dispatcherCfs::doAction('adminMainNavClassAdd'); ?>">
                <ul>
        					<?php foreach ($this->tabs as $tabKey => $tab) { ?>
        						<?php if (isset($tab['hidden']) && $tab['hidden']) {
                continue;
              } ?>
...
<div id="cfsOptInProWnd" style="display: none;" title="<?php _e('Improve Free version', CFS_LANG_CODE); ?>">
	<p id="cfsOptWndTemplateTxt" style="display: none;">
		<?php printf(
    __(
      'Please be advised that this template with all other options and PRO templates is available only in <a target="_blank" href="%s">PRO version</a>. You can <a target="_blank" href="%s" class="button">Get PRO</a> today and get this and other PRO templates and features for your Forms!',
      CFS_LANG_CODE,
    ),
    $this->mainLink,
    $this->mainLink,
